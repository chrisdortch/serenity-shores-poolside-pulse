const VERSION = '23';
const DISPLAY_VERSION = 'v23';
const PIN = '7900';
const KEY = 'poolside-pulse-v23';
const DEVICE_KEY = 'poolside-pulse-v23-device-id';
const HANDLED_KEY = 'poolside-pulse-v23-handled-events';
const LOG_CLEAR_KEY = 'poolside-pulse-v23-log-cleared-at';
const RECEIVER_SESSION_KEY = 'poolside-pulse-v23-receiver-session-started-at';
const SPOTIFY_TOKEN_KEY = 'poolside-pulse-v23-spotify-token';
...
const RETRYABLE_MUSIC_COMMANDS = new Set(['spotify-play', 'quiet-bed-play', 'play', 'suno', 'suno-cue', 'song']);
...
const BASE = {
  ...
  autoStart: true,
  autoStop: true,
  v23DefaultsApplied: true,
  v23VolumeDefaultsApplied: V23_VOLUME_DEFAULTS_ID,
  v20DefaultsApplied: true,
  v20VolumeDefaultsApplied: 'migrated-to-v23',
...
function normalize(raw) {
  ...
  const importedFromV23 = String(source.version || '') === VERSION || source.v23DefaultsApplied || source.v20DefaultsApplied;
  const importedFromRecentPoolside = importedFromV23 || source.v18DefaultsApplied;

  // V23 uses Suno/direct audio as the guaranteed quiet bed because that path
  // stays inside Poolside Pulse's Web Audio graph. Spotify remains controllable,
  // but iOS does not allow browser JavaScript to attenuate its local volume.
  s.musicProvider = source.musicProvider === 'spotify' ? 'spotify' : 'suno';
  ...
  if (source.v23VolumeDefaultsApplied !== V23_VOLUME_DEFAULTS_ID) {
    ...
    s.setupNotice = '';
    s.spotifyLastError = '';
    s.v23VolumeDefaultsApplied = V23_VOLUME_DEFAULTS_ID;
    s.v20VolumeDefaultsApplied = 'migrated-to-v23';
  }
...
async function runCommand(command) {
  ...
  if (command.type === 'quiet-bed-play') S.musicProvider = 'suno';
  else if (['spotify-play', 'spotify-volume'].includes(command.type)) S.musicProvider = 'spotify';
  ...
  if (command.type === 'spotify-play') {
  ...
  } else if (command.type === 'quiet-bed-play') {
    S.musicProvider = 'suno';
    if (Number.isFinite(Number(command.sunoVolume))) S.sunoVolume = sunoVolumePercent(command.sunoVolume);
    if (Number.isFinite(Number(command.spokenGain))) {
      S.spokenGain = spokenGainValue(command.spokenGain);
      S.announcementGain = announcementGainFromSpokenGain(S.spokenGain);
    }
    ...
    else throw Error('V23 Suno cues require a pasted Suno or direct audio URL.');
...
function readMusicSettings() {
  if ($('musicProvider')) S.musicProvider = val('musicProvider') === 'spotify' ? 'spotify' : 'suno';
  else if (String(S.playlistUrl || '').trim() && S.activeMusicProvider !== 'spotify') S.musicProvider = 'suno';
  else S.musicProvider = S.musicProvider === 'spotify' ? 'spotify' : 'suno';
...
function receiverAudioReady() {
  const status = typeof window.__poolsideV23AudioStatus === 'function' ? window.__poolsideV23AudioStatus() : null;
...
async function ensureReceiverAudio(reason = 'receiver action', options = {}) {
  ...
  const attempts = [];
  if (typeof window.__poolsideV23UnlockAudio === 'function') attempts.push(window.__poolsideV23UnlockAudio(reason, unlockOptions));
  if (unlockOptions.audible || typeof window.__poolsideV23UnlockAudio !== 'function') attempts.push(fallbackUnlockReceiverAudio(reason, unlockOptions));
  if (attempts.length) {
...
async function playQuietBedUrl(url = S.playlistUrl || S.quickMusicUrl, push = true, options = {}) {
  ...
  if (S.screen !== 'home' && push) {
    ...
    }, `Quiet bed command sent: ${sourceLabel('suno', raw)} at ${sunoVolumePercent(S.sunoVolume)}%.`);
    return;
  }
  ...
  S.activeMusicLabel = `Quiet bed playing at ${sunoVolumePercent(S.sunoVolume)}%: ${title}`;
  ...
  const playbackPromise = playReceiverAudioUrl(chosen.audioUrl, {
    ...
    if (activeForegroundPlayback?.id === playback.id) {
      ...
      if (!playback.stopped) {
        S.intent = 'stopped';
        S.activeMusicLabel = `Quiet bed ended: ${title}`;
        await pushState('Receiver quiet bed ended.', { render: false });
        renderWhenIdle();
      }
    }
  });
  playbackPromise.catch(() => {});
  await pushState(options.restore ? 'Receiver quiet bed restored.' : 'Receiver quiet bed started.', { render: false });
  renderWhenIdle();
}

async function duckQuietBedForAnnouncement() {
  const playback = activeForegroundPlayback;
  if (!playback || playback.kind !== 'quiet-bed') return null;
  const snapshot = {
    wasPlaying: true,
    url: playback.url,
  ...
  if (typeof window.__poolsideV23StopAudio === 'function') {
    try { window.__poolsideV23StopAudio('voice announcement'); } catch {}
  }
  activeForegroundPlayback = null;
  S.activeMusicLabel = `Quiet bed paused for loud voice: ${playback.title || 'music bed'}`;
  logEvent('music', 'Quiet bed paused for loud voice', playback.title || '');
  return snapshot;
}

async function restoreQuietBedAfterAnnouncement(snapshot) {
  if (!snapshot?.wasPlaying || manualMusicHoldActive()) return;
  try {
    await wait(VOICE_TAKEOVER_POST_ROLL_MS);
    await playQuietBedUrl(snapshot.url, false, { restore: true });
  } catch (error) {
    logEvent('music', 'Quiet bed restore failed', error.message || String(error), { url: snapshot.url || '' });
    setFeedback(`Announcement finished; quiet bed restore failed: ${error.message}`, false);
  }
}
...
async function stopReceiverForegroundAudio(reason = 'stop command') {
  ...
  if (typeof window.__poolsideV23StopAudio === 'function') {
    try { stopped = !!window.__poolsideV23StopAudio(reason) || stopped; } catch {}
  }
...
async function pauseSuno(push = true) {
  ...
  const stoppedForeground = await stopReceiverForegroundAudio('Pause quiet bed command');
  if (stoppedForeground) {
    S.intent = 'paused';
    setManualMusicHold('Quiet bed paused manually');
    logEvent('pause', 'Quiet bed paused', '');
    setFeedback('Quiet bed paused on receiver.', true);
    await pushState('Receiver quiet bed pause logged.', { render: false });
    renderWhenIdle();
    return;
  }
...
async function playAnyMusicUrl(url = S.quickMusicUrl || activeProviderUrl(), push = true) {
  ...
  S.musicProvider = 'suno';
  await playQuietBedUrl(raw, push);
}
...
function applyReceiverAudioSettings(reason = 'local setting', options = {}) {
  ...
  if (activeForegroundPlayback?.kind === 'quiet-bed' && typeof window.__poolsideV23SetActiveVolume === 'function') {
    try { window.__poolsideV23SetActiveVolume(musicGain(audio.sunoVolume)); } catch {}
  }
...
async function setupLoudVoiceReceiver() {
  ...
  S.announcementGain = announcementGainFromSpokenGain(S.spokenGain);
  S.voiceMode = 'ai';
  S.iosVolumeBridgeStatus = `V23 Loud Voice Setup active. ${IOS_VOLUME_BRIDGE_FIX_TEXT}`;
...
function registerSpotifyListeners() {
  spotifyPlayer.addListener('ready', ({ device_id }) => {
    ...
    S.spotifyDeviceName = 'Poolside Pulse V23 Receiver';
...
async function primeSpotifyPlayer() {
  ...
  spotifyPrimePromise = (async () => {
    ...
    spotifyPlayer = new Spotify.Player({
      name: 'Poolside Pulse V23 Receiver',
...
async function startSpotifyReceiver({ fromTap = false } = {}) {
  ...
  S.spotifyDeviceName = S.spotifyDeviceName || 'Poolside Pulse V23 Receiver';
  S.spotifyReceiverReadyAt = Date.now();
...
async function holdSpotifyBedVolume(reason = 'receiver volume hold') {
  ...
  try {
    ...
    if (now - spotifyVolumeWatchLastLogAt > SPOTIFY_VOLUME_WATCH_LOG_MS) {
      ...
      logEvent('spotify', 'Spotify volume hold active', `${S.spotifyVolume}%${result?.method ? ` via ${result.method}` : ''}${isIOSLikeBrowser() ? '; V23 uses quiet-bed mode for guaranteed low music and pauses Spotify during voice' : ''}`);
...
async function playReceiverAudioUrl(url, options = {}) {
  ...
  if (typeof window.__poolsideV23PlayAudioUrl === 'function') {
    return await window.__poolsideV23PlayAudioUrl(raw, {
      label,
      volume,
      gain: volume,
      minGain: 0,
  ...
  if (typeof window.__poolsideV23PlayAnnouncementBlob === 'function') {
    try {
      const response = await fetch(raw, { cache: 'no-store' });
      if (!response.ok) throw Error(`Audio URL returned HTTP ${response.status}`);
      const blob = await response.blob();
      return await window.__poolsideV23PlayAnnouncementBlob(blob, {
        label,
        volume,
        gain: volume,
        minGain: 0,
...
async function performAnnouncement(text, options = {}) {
  ...
  try {
    ...
    await restoreSpotifyAfterAnnouncement(spotifySnapshot);
    await restoreSunoAfterAnnouncement(sunoSnapshot);
    await restoreQuietBedAfterAnnouncement(quietBedSnapshot);
    speaking = false;
    localSave();
    renderWhenIdle();
  }
...
function commandPage() {
  ...
  return shell(`
    ...
    <section class="panel controlDeck">
      ...
      <div class="bigControls"><button id="playCmd">Play Quiet Bed</button><button id="pauseCmd" class="secondary">Pause</button><button id="skipCmd" class="secondary">Restart Bed</button><button id="stopCmd" class="danger">Stop</button><button id="checkWeatherCmd" class="secondary">Check Weather</button></div>
      <div class="volumeStrip">
        <label><span>Quiet Bed Volume <output id="sunoVolumeOut">${esc(S.sunoVolume)}%</output></span><input id="sunoVolume" type="range" min="${SUNO_VOLUME_SLIDER_MIN}" max="${SUNO_VOLUME_SLIDER_MAX}" step="1" value="${esc(S.sunoVolume)}"></label>
...
function musicPageV23() {
  ...
  return shell(`
    <section class="panel">
      ...
      <div class="sectionHead"><h2>Audio Balance Manager</h2><span class="pill">V23</span></div>
      <div class="grid4">
        <label><span>Quiet Bed Volume <output id="sunoVolumeOut">${esc(S.sunoVolume)}%</output></span><input id="sunoVolume" type="range" min="${SUNO_VOLUME_SLIDER_MIN}" max="${SUNO_VOLUME_SLIDER_MAX}" step="1" value="${esc(S.sunoVolume)}"></label>
        <label><span>Spotify Gain <output id="spotifyGainOut">${esc(spotifyGainLabel())}</output></span><input id="spotifyGain" type="range" min="${MIN_SPOTIFY_GAIN}" max="${MAX_SPOTIFY_GAIN}" step="25" value="${esc(S.spotifyGain)}"></label>
        <label><span>Spoken Gain <output id="spokenGainOut">${esc(spokenGainLabel())}</output></span><input id="spokenGain" type="range" min="${MIN_SPOKEN_GAIN}" max="${MAX_SPOKEN_GAIN}" step="25" value="${esc(S.spokenGain)}"></label>
      ...
      <div class="grid2">
        <label><span>Spotify Volume Request <output id="spotifyVolumeOut">${esc(S.spotifyVolume)}%</output></span><input id="spotifyVolume" type="range" min="${SPOTIFY_VOLUME_SLIDER_MIN}" max="${SPOTIFY_VOLUME_SLIDER_MAX}" step="1" value="${esc(S.spotifyVolume)}"></label>
      ...
      <p class="muted">Spotify volume requests are retained for non-iOS or external Spotify Connect devices, but the reliable iPhone receiver gap comes from the quiet bed path above.</p>
      ${spotifyDiagnostics()}
...
function render() {
  try {
    ...
    app.innerHTML = S.screen === 'home'
      ...
      : !S.admin
        ? login()
        : ({ command: commandPage, music: musicPageV23, party: () => schedulePage('party'), schedule: () => schedulePage('daily'), weather: weatherPage, voice: voicePage, hours: hoursPage }[S.tab] || commandPage)();
    bind();
  } catch (error) {
...
function bind() {
  ...
  wire('spotifyVolumeApply', async () => {
    ...
    if ($('spotifyGain')) await applySpotifyVolume(true);
  });
  ...
  wire('savePastedUrl', async () => {
    ...
    if (provider === 'suno') {
      S.musicProvider = 'suno';
      S.playlistUrl = raw;
      rememberActiveSource('suno', raw, 'saved quiet bed');
      await save('Quiet bed URL saved. This is the guaranteed low-volume music path.');
      return;
    } else {
      S.musicProvider = 'spotify';
      S.spotifyUrl = raw;
  ...
  wire('makeReceiver', async () => {
    readMusicSettings();
    setFeedback('Command stays Command in V23. Open Home on the speaker-connected phone, then tap Start Receiver once. Use Play Quiet Bed for the real low-volume path.', true);
  });
...
window.addEventListener('poolside-v23-audio-status', event => {
