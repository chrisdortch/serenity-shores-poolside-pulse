const VERSION = '21';
const DISPLAY_VERSION = 'v21';
const PIN = '7900';
const KEY = 'poolside-pulse-v21';
const DEVICE_KEY = 'poolside-pulse-v21-device-id';
const HANDLED_KEY = 'poolside-pulse-v21-handled-events';
const LOG_CLEAR_KEY = 'poolside-pulse-v21-log-cleared-at';
const RECEIVER_SESSION_KEY = 'poolside-pulse-v21-receiver-session-started-at';
const SPOTIFY_TOKEN_KEY = 'poolside-pulse-v21-spotify-token';
const IOS_VOLUME_BRIDGE_KEY = 'poolside-pulse-v21-ios-volume-bridge-enabled';
const IOS_VOLUME_BRIDGE_NAME_KEY = 'poolside-pulse-v21-ios-volume-bridge-name';
const IOS_VOLUME_BRIDGE_MODE_KEY = 'poolside-pulse-v21-ios-volume-bridge-mode-id';
const IOS_VOLUME_BRIDGE_MODE_ID = '2026-07-10-v21-manager-gain-gap';
const APP_QUERY = '?v=21';
...
const DEFAULT_SPOTIFY_VOLUME = 1;
const DEFAULT_SPOTIFY_DUCKED_VOLUME = 0;
const DEFAULT_SUNO_VOLUME = 15;
...
const DEFAULT_SPOTIFY_GAIN = -500;
const MIN_SPOTIFY_GAIN = -500;
const MAX_SPOTIFY_GAIN = 0;
const DEFAULT_SPOKEN_GAIN = 500;
const MIN_SPOKEN_GAIN = 0;
const MAX_SPOKEN_GAIN = 500;
const DEFAULT_ANNOUNCEMENT_GAIN = 40;
const MAX_ANNOUNCEMENT_GAIN = 40;
const V21_VOLUME_DEFAULTS_ID = '2026-07-10-v21-manager-gain-gap';
...
const IOS_VOLUME_BRIDGE_FIX_TEXT = 'V21 uses Manager gains: Spotify defaults to -500/1%, spoken word defaults to +500/max PA, music pauses during spoken commands, then restores.';
...
const BASE = {
  ...
  autoStart: true,
  autoStop: true,
  v21DefaultsApplied: true,
  v21VolumeDefaultsApplied: V21_VOLUME_DEFAULTS_ID,
  v20DefaultsApplied: true,
  v20VolumeDefaultsApplied: 'migrated-to-v21',
...
function normalize(raw) {
  ...
  const importedFromV21 = String(source.version || '') === VERSION || source.v21DefaultsApplied || source.v20DefaultsApplied;
  const importedFromRecentPoolside = importedFromV21 || source.v18DefaultsApplied;

  // V21 uses Spotify as the continuous bed. Suno is a cue/announcement source,
  ...
  if (source.v21VolumeDefaultsApplied !== V21_VOLUME_DEFAULTS_ID) {
    ...
    s.iosVolumeBridgeStatus = `V21 clear PA voice mode: ${IOS_VOLUME_BRIDGE_FIX_TEXT}`;
    ...
    s.setupNotice = '';
    s.spotifyLastError = '';
    s.v21VolumeDefaultsApplied = V21_VOLUME_DEFAULTS_ID;
    s.v20VolumeDefaultsApplied = 'migrated-to-v21';
  }
...
async function runCommand(command) {
  ...
  if (command.type === 'spotify-play') {
    ...
    else throw Error('V21 Suno cues require a pasted Suno or direct audio URL.');
...
function receiverAudioReady() {
  const status = typeof window.__poolsideV21AudioStatus === 'function' ? window.__poolsideV21AudioStatus() : null;
...
async function ensureReceiverAudio(reason = 'receiver action', options = {}) {
  ...
  const attempts = [];
  if (typeof window.__poolsideV21UnlockAudio === 'function') attempts.push(window.__poolsideV21UnlockAudio(reason, unlockOptions));
  if (unlockOptions.audible || typeof window.__poolsideV21UnlockAudio !== 'function') attempts.push(fallbackUnlockReceiverAudio(reason, unlockOptions));
  if (attempts.length) {
...
async function stopReceiverForegroundAudio(reason = 'stop command') {
  ...
  if (typeof window.__poolsideV21StopAudio === 'function') {
    try { stopped = !!window.__poolsideV21StopAudio(reason) || stopped; } catch {}
  }
...
async function setupLoudVoiceReceiver() {
  ...
  S.announcementGain = announcementGainFromSpokenGain(S.spokenGain);
  S.voiceMode = 'ai';
  S.iosVolumeBridgeStatus = `V21 Loud Voice Setup active. ${IOS_VOLUME_BRIDGE_FIX_TEXT}`;
...
function registerSpotifyListeners() {
  spotifyPlayer.addListener('ready', ({ device_id }) => {
    ...
    S.spotifyDeviceName = 'Poolside Pulse V21 Receiver';
...
async function primeSpotifyPlayer() {
  ...
  spotifyPrimePromise = (async () => {
    ...
    spotifyPlayer = new Spotify.Player({
      name: 'Poolside Pulse V21 Receiver',
...
async function startSpotifyReceiver({ fromTap = false } = {}) {
  ...
  S.spotifyDeviceName = S.spotifyDeviceName || 'Poolside Pulse V21 Receiver';
  S.spotifyReceiverReadyAt = Date.now();
...
async function holdSpotifyBedVolume(reason = 'receiver volume hold') {
  ...
  try {
    ...
    if (now - spotifyVolumeWatchLastLogAt > SPOTIFY_VOLUME_WATCH_LOG_MS) {
      ...
      logEvent('spotify', 'Spotify volume hold active', `${S.spotifyVolume}%${result?.method ? ` via ${result.method}` : ''}${isIOSLikeBrowser() ? '; V21 pauses music during voice and restores it after' : ''}`);
...
async function duckSpotifyForAnnouncement(options = {}) {
  ...
  try {
    ...
    if (pauseSpotify) {
      ...
      if (!pauseMethod && snapshot.wasPlaying) throw Error('Spotify did not accept a pause command for foreground music.');
      if (!pauseMethod) return snapshot;
      snapshot.pausedForDuck = true;
      snapshot.wasPlaying = true;
      snapshot.duckMethod = `pause via ${pauseMethod}`;
      logEvent('spotify', 'Spotify paused for voice takeover', `Paused while receiver speech plays via ${pauseMethod}.`);
      setFeedback('Spotify paused while the spoken command plays; it will restore afterward.', true);
      return snapshot;
    }
    let localSet = false;
    let remoteSet = false;
...
async function playReceiverAudioUrl(url, options = {}) {
  ...
  if (typeof window.__poolsideV21PlayAudioUrl === 'function') {
    return await window.__poolsideV21PlayAudioUrl(raw, {
      label,
      volume,
      gain: volume,
      minGain: 0,
  ...
  if (typeof window.__poolsideV21PlayAnnouncementBlob === 'function') {
    try {
      const response = await fetch(raw, { cache: 'no-store' });
      if (!response.ok) throw Error(`Audio URL returned HTTP ${response.status}`);
      const blob = await response.blob();
      return await window.__poolsideV21PlayAnnouncementBlob(blob, {
        label,
        volume,
        gain: volume,
        minGain: 0,
...
async function performAnnouncement(text, options = {}) {
  ...
  const sunoSnapshot = await duckSunoForAnnouncement();
  try {
    if (spotifySnapshot?.pausedForDuck || sunoSnapshot?.wasPlaying) {
      S.audioStatus = 'Voice announcement has taken over receiver audio.';
      await wait(350);
    }
...
function musicPageV21() {
  ...
  return shell(`
    <section class="panel">
      ...
      <div class="sectionHead"><h2>Audio Balance Manager</h2><span class="pill">V21</span></div>
      <div class="grid4">
...
function render() {
  try {
    ...
    app.innerHTML = S.screen === 'home'
      ...
      : !S.admin
        ? login()
        : ({ command: commandPage, music: musicPageV21, party: () => schedulePage('party'), schedule: () => schedulePage('daily'), weather: weatherPage, voice: voicePage, hours: hoursPage }[S.tab] || commandPage)();
    bind();
  } catch (error) {
...
function bind() {
  ...
  wire('useSuno', async () => {
    ...
    S.musicProvider = 'spotify';
    rememberActiveSource('spotify', S.spotifyUrl, 'selected');
    await save('V21 keeps Spotify as the bed music. Paste a Suno URL and use Play Saved Suno Cue for Suno.');
  });
  ...
  wire('makeReceiver', async () => {
    readMusicSettings();
    setFeedback('Command stays Command in V21. Open Home on the speaker-connected phone, then tap Start Receiver + Play Spotify there.', true);
  });
...
window.addEventListener('poolside-v21-audio-status', event => {
