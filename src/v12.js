import { DEFAULT_PARTY_SCHEDULE } from './v12-party-defaults.js';

const VERSION = '12';
const PIN = '7900';
const KEY = 'poolside-pulse-v12';
const DEVICE_KEY = 'poolside-pulse-v12-device-id';
const HANDLED_KEY = 'poolside-pulse-v12-handled-events';
const LOG_CLEAR_KEY = 'poolside-pulse-v12-log-cleared-at';
const RECEIVER_SESSION_KEY = 'poolside-pulse-v12-receiver-session-started-at';
const RECEIVER_ENABLED_KEY = 'poolside-pulse-v12-receiver-enabled';
const SPOTIFY_TOKEN_KEY = 'poolside-pulse-v12-spotify-token';
const APP_QUERY = '?v=12-lake123';
...
function helperAudioStatus() {
  const fn = window.__poolsideV12AudioStatus || window.__poolsideV9AudioStatus;
  return typeof fn === 'function' ? fn() : null;
}

function helperUnlockAudio(reason, options) {
  const fn = window.__poolsideV12UnlockAudio || window.__poolsideV9UnlockAudio;
  return typeof fn === 'function' ? fn(reason, options) : null;
}

function helperPlayAnnouncementBlob(blob, options) {
  const fn = window.__poolsideV12PlayAnnouncementBlob || window.__poolsideV9PlayAnnouncementBlob;
...
function registerSpotifyListeners() {
  spotifyPlayer.addListener('ready', ({ device_id }) => {
    ...
    S.spotifyDeviceName = 'Poolside Pulse V12 Receiver';
...
async function primeSpotifyPlayer() {
  ...
  spotifyPrimePromise = (async () => {
    ...
    spotifyPlayer = new Spotify.Player({
      name: 'Poolside Pulse V12 Receiver',
...
async function startSpotifyReceiver({ fromTap = false } = {}) {
  ...
  S.spotifyDeviceName = S.spotifyDeviceName || 'Poolside Pulse V12 Receiver';
  S.spotifyReceiverReadyAt = Date.now();
  S.receiverStatus = 'Spotify receiver active.';
  S.receiverLastSeen = stamp();
  receiverActive = true;
...
async function restorePartyDefaults() {
  if (!confirm('Restore the default Version 12 party schedule? This replaces the editable party cue list on this app.')) return;
  S.partySchedule = normalizePartySchedule(clone(DEFAULT_PARTY_SCHEDULE));
  S.partySelectedCueId = S.partySchedule.cues[0]?.id || '';
  S.partyImportText = '';
  S.partyExportText = '';
  await save('Default Version 12 party schedule restored.');
}
...
function bind() {
  ...
  wire('partyRestart', async () => {
    if (!confirm('Restart the Version 12 party schedule from the first enabled cue?')) return;
...
window.addEventListener('poolside-v12-audio-status', handleHelperAudioStatus);
