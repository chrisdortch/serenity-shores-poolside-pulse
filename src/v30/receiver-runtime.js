...
const SESSION_KEY = 'poolside-pulse-v30-receiver-session';
...
export class ReceiverRuntime {
  ...
  async updateReceiverDetail(detail, audioMode = '') {
    ...
    await this.store.mutate(draft => {
      if (draft.receiver?.id !== this.deviceId || draft.receiver?.sessionId !== this.sessionId) return draft;
      draft.receiver = renewReceiverLease(draft.receiver, this.now(), { detail, audioMode: audioMode || draft.receiver.audioMode, ...this.spotifyLeasePatch() });
      return draft;
  ...
  async sendCommand(type, payload = {}, message = 'Command sent.') {
    ...
    await this.store.mutate(draft => {
      ...
      if (type === 'play-spotify' && draft.receiver?.spotifyStatus !== 'ready') {
        const detail = draft.receiver?.spotifyDetail || 'Open v30 on the speaker device, verify Spotify access, and tap Connect Spotify Receiver.';
    ...
    try {
      try {
        ...
        failureMeta = {
          ...
          errorOperation: String(caught?.spotifyOperation || '').slice(0, 120),
      ...
      await this.store.mutate(draft => {
        ...
        if (event.type === 'play-spotify' && draft.receiver?.id === this.deviceId && draft.receiver?.sessionId === this.sessionId) {
          draft.receiver = renewReceiverLease(draft.receiver, completedAt, this.spotifyLeasePatch());
        }
        draft.activityLog = [
  ...
  async playSpotify(url, { volumePercent = null, volumeMode = 'global', scheduledItemId = '', scheduledRunToken = '', persistSource = true } = {}) {
    ...
    const physical = await this.serializeAudio(async () => {
      ...
      try {
        ...
        if (restoreError) {
          throw spotifyErrorWithContext(error, `${error.message} Controlled music also could not be restored: ${restoreError}`);
        }
        throw error;
      }
