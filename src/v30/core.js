...
function normalizeSequenceActive(active) {
  ...
  return {
    ...
    expectedProvider: ['controlled', 'spotify'].includes(requestedProvider) ? requestedProvider : '',
...
export function makeReceiverLease({ deviceId, sessionId, name = 'Speaker Receiver', platform = '', audioMode = '', spotifyStatus = 'login-required', spotifyDetail = '', spotifyVerifiedAt = 0 }, now = Date.now()) {
  return {
    id: String(deviceId || ''),
...
export function createTargetedEvent(type, payload, receiver, now = Date.now()) {
  if (!receiverOnline(receiver, now)) throw new Error('The speaker receiver is offline. Open v30 on the speaker device and tap Start Receiver.');
