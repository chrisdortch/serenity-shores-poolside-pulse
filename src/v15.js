import { DEFAULT_PARTY_SCHEDULE } from './v15-party-defaults.js';

const VERSION = '15';
const PIN = '7900';
const KEY = 'poolside-pulse-v15';
const DEVICE_KEY = 'poolside-pulse-v15-device-id';
const HANDLED_KEY = 'poolside-pulse-v15-handled-events';
const LOG_CLEAR_KEY = 'poolside-pulse-v15-log-cleared-at';
const RECEIVER_SESSION_KEY = 'poolside-pulse-v15-receiver-session-started-at';
const RECEIVER_ENABLED_KEY = 'poolside-pulse-v15-receiver-enabled';
const SPOTIFY_TOKEN_KEY = 'poolside-pulse-v15-spotify-token';
const APP_QUERY = '?v=15';
...
function normalize(raw) {
  ...
  if (migratedDeletedSuno && !s.tracks.some(item => item.audioUrl)) {
    ...
    s.current = 0;
    s.lastError = 'The old default Suno playlist was deleted on Suno. Paste a valid Suno playlist, Suno song, or direct audio URL to use Suno.';
...
function helperAudioStatus() {
  const fn = window.__poolsideV15AudioStatus || window.__poolsideV9AudioStatus;
  return typeof fn === 'function' ? fn() : null;
}

function helperUnlockAudio(reason, options) {
  const fn = window.__poolsideV15UnlockAudio || window.__poolsideV9UnlockAudio;
  return typeof fn === 'function' ? fn(reason, options) : null;
}

function helperPlayAnnouncementBlob(blob, options) {
  const fn = window.__poolsideV15PlayAnnouncementBlob || window.__poolsideV9PlayAnnouncementBlob;
...
function spotifyCooldownError(message = '') {
  ...
  const error = Error(message || `Spotify is rate-limiting commands. V15 will retry in about ${seconds} second(s).`);
...
function registerSpotifyListeners() {
  spotifyPlayer.addListener('ready', ({ device_id }) => {
    ...
    S.spotifyDeviceName = 'Poolside Pulse V15 Receiver';
...
async function primeSpotifyPlayer() {
  ...
  spotifyPrimePromise = (async () => {
    ...
    spotifyPlayer = new Spotify.Player({
      name: 'Poolside Pulse V15 Receiver',
...
async function startSpotifyReceiver({ fromTap = false } = {}) {
  ...
  S.spotifyDeviceName = S.spotifyDeviceName || 'Poolside Pulse V15 Receiver';
  S.spotifyReceiverReadyAt = Date.now();
  S.receiverStatus = 'Spotify receiver active.';
  S.receiverLastSeen = stamp();
  receiverActive = true;
...
async function restorePartyDefaults() {
  if (!confirm('Restore the default Version 15 party schedule? This replaces the editable party cue list on this app.')) return;
  S.partySchedule = normalizePartySchedule(clone(DEFAULT_PARTY_SCHEDULE));
  S.partySelectedCueId = S.partySchedule.cues[0]?.id || '';
  S.partyImportText = '';
  S.partyExportText = '';
  await save('Default Version 15 party schedule restored.');
}
...
function bind() {
  ...
  wire('partyRestart', async () => {
    if (!confirm('Restart the Version 15 party schedule from the first enabled cue?')) return;
...
window.addEventListener('poolside-v15-audio-status', handleHelperAudioStatus);
