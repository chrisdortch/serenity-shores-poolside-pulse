import { DEFAULT_PARTY_SCHEDULE } from './v14-party-defaults.js';

const VERSION = '14';
const PIN = '7900';
const KEY = 'poolside-pulse-v14';
const DEVICE_KEY = 'poolside-pulse-v14-device-id';
const HANDLED_KEY = 'poolside-pulse-v14-handled-events';
const LOG_CLEAR_KEY = 'poolside-pulse-v14-log-cleared-at';
const RECEIVER_SESSION_KEY = 'poolside-pulse-v14-receiver-session-started-at';
const RECEIVER_ENABLED_KEY = 'poolside-pulse-v14-receiver-enabled';
const SPOTIFY_TOKEN_KEY = 'poolside-pulse-v14-spotify-token';
const APP_QUERY = '?v=14';
  ...
  if (command.type === 'spotify-play') {
    ...
    if (shouldTouchSpotify) {
      try {
        ...
        throw actionNeededError(`Spotify music volume did not apply on this receiver: ${error.message || String(error)}. Tap Activate Receiver + Spotify on the speaker-connected receiver, then send Set Music Volume again.`);
      }
...
function helperAudioStatus() {
  const fn = window.__poolsideV14AudioStatus || window.__poolsideV9AudioStatus;
  return typeof fn === 'function' ? fn() : null;
}

function helperUnlockAudio(reason, options) {
  const fn = window.__poolsideV14UnlockAudio || window.__poolsideV9UnlockAudio;
  return typeof fn === 'function' ? fn(reason, options) : null;
}

function helperPlayAnnouncementBlob(blob, options) {
  const fn = window.__poolsideV14PlayAnnouncementBlob || window.__poolsideV9PlayAnnouncementBlob;
...
async function applySpotifyVolume(push = true) {
  ...
  if (shouldTouchSpotify) {
    try {
      ...
      throw actionNeededError(`Spotify music volume did not apply on this receiver: ${error.message || String(error)}. Tap Activate Receiver + Spotify once, then try the slider again.`);
    }
...
function registerSpotifyListeners() {
  spotifyPlayer.addListener('ready', ({ device_id }) => {
    ...
    S.spotifyDeviceName = 'Poolside Pulse V14 Receiver';
...
async function primeSpotifyPlayer() {
  ...
  spotifyPrimePromise = (async () => {
    ...
    spotifyPlayer = new Spotify.Player({
      name: 'Poolside Pulse V14 Receiver',
...
async function startSpotifyReceiver({ fromTap = false } = {}) {
  ...
  S.spotifyDeviceName = S.spotifyDeviceName || 'Poolside Pulse V14 Receiver';
  S.spotifyReceiverReadyAt = Date.now();
  S.receiverStatus = 'Spotify receiver active.';
  S.receiverLastSeen = stamp();
  receiverActive = true;
...
async function sendSpotifyPlayback(playUrl, deviceId) {
  ...
  await spotifySetVolume(S.spotifyVolume, deviceId, { persist: false, preferKnown: true });
...
async function restorePartyDefaults() {
  if (!confirm('Restore the default Version 14 party schedule? This replaces the editable party cue list on this app.')) return;
  S.partySchedule = normalizePartySchedule(clone(DEFAULT_PARTY_SCHEDULE));
  S.partySelectedCueId = S.partySchedule.cues[0]?.id || '';
  S.partyImportText = '';
  S.partyExportText = '';
  await save('Default Version 14 party schedule restored.');
}
...
function bind() {
  ...
  wire('partyRestart', async () => {
    if (!confirm('Restart the Version 14 party schedule from the first enabled cue?')) return;
...
window.addEventListener('poolside-v14-audio-status', handleHelperAudioStatus);
