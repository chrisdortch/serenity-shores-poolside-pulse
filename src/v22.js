const VERSION = '22';
const DISPLAY_VERSION = 'v22';
const PIN = '7900';
const KEY = 'poolside-pulse-v22';
const DEVICE_KEY = 'poolside-pulse-v22-device-id';
const HANDLED_KEY = 'poolside-pulse-v22-handled-events';
const LOG_CLEAR_KEY = 'poolside-pulse-v22-log-cleared-at';
const RECEIVER_SESSION_KEY = 'poolside-pulse-v22-receiver-session-started-at';
const SPOTIFY_TOKEN_KEY = 'poolside-pulse-v22-spotify-token';
const IOS_VOLUME_BRIDGE_KEY = 'poolside-pulse-v22-ios-volume-bridge-enabled';
const IOS_VOLUME_BRIDGE_NAME_KEY = 'poolside-pulse-v22-ios-volume-bridge-name';
const IOS_VOLUME_BRIDGE_MODE_KEY = 'poolside-pulse-v22-ios-volume-bridge-mode-id';
const IOS_VOLUME_BRIDGE_MODE_ID = '2026-07-10-v22-max-gap-takeover';
const APP_QUERY = '?v=22';
...
const LEGACY_TOKEN_KEYS = [
  'poolside-pulse-v21-spotify-token',
  'poolside-pulse-v20-spotify-token',
  'poolside-pulse-v18-spotify-token',
...
const DEFAULT_SPOTIFY_GAIN = -800;
const MIN_SPOTIFY_GAIN = -800;
const MAX_SPOTIFY_GAIN = 0;
const DEFAULT_SPOKEN_GAIN = 800;
const MIN_SPOKEN_GAIN = 0;
const MAX_SPOKEN_GAIN = 800;
const DEFAULT_ANNOUNCEMENT_GAIN = 64;
const MAX_ANNOUNCEMENT_GAIN = 64;
const V22_VOLUME_DEFAULTS_ID = '2026-07-10-v22-max-gap-takeover';
...
const IOS_VOLUME_BRIDGE_FIX_TEXT = 'V22 Max Gap uses Manager gains: Spotify defaults to -800/0%, spoken word defaults to +800/max PA+, music is silenced and paused before speech, then restores after a hold.';
const SPOTIFY_VOLUME_VERIFY_TOLERANCE = 1;
const VOICE_TAKEOVER_PRE_ROLL_MS = 850;
const VOICE_TAKEOVER_POST_ROLL_MS = 1100;
const SPOTIFY_SILENCE_VOLUME = 0;
const SPOTIFY_SILENCE_REPEAT_MS = 220;
const EVENT_TTL_MS = 90 * 60 * 1000;
const EVENT_LIMIT = 120;
const LOG_LIMIT = 180;
const EVENT_RETRY_MS = 9000;
const RECEIVER_EVENT_GRACE_MS = 3000;
const SCHEDULE_CATCHUP_MS = 10 * 60 * 1000;
const RETRYABLE_MUSIC_COMMANDS = new Set(['spotify-play', 'play', 'suno', 'suno-cue', 'song']);
const SPOTIFY_DEVICE_COMMANDS = new Set(['spotify-play', 'play', 'pause', 'stop', 'skip', 'spotify-volume']);
...
const BASE = {
  ...
  spotifyRedirectUri: '',
  spotifyVolume: DEFAULT_SPOTIFY_VOLUME,
  spotifyGain: DEFAULT_SPOTIFY_GAIN,
  spotifyDuckedVolume: DEFAULT_SPOTIFY_DUCKED_VOLUME,
  announcementGain: DEFAULT_ANNOUNCEMENT_GAIN,
  spokenGain: DEFAULT_SPOKEN_GAIN,
  sunoVolume: DEFAULT_SUNO_VOLUME,
  sunoDuckedVolume: 2,
  ...
  spotifyLastError: '',
  iosVolumeBridgeStatus: '',
  iosVolumeBridgeLastTarget: '',
  iosVolumeBridgeLastAt: 0,
  anns: DEFAULT_ANNS,
  selected: 'welcome',
  quickText: '',
  guestName: '',
  ...
  autoStart: true,
  autoStop: true,
  v22DefaultsApplied: true,
  v22VolumeDefaultsApplied: V22_VOLUME_DEFAULTS_ID,
  v20DefaultsApplied: true,
  v20VolumeDefaultsApplied: 'migrated-to-v22',
...
function spotifyGainValue(value = S?.spotifyGain) {
  return clampNumber(value, MIN_SPOTIFY_GAIN, MAX_SPOTIFY_GAIN, DEFAULT_SPOTIFY_GAIN);
}

function spokenGainValue(value = S?.spokenGain) {
  return clampNumber(value, MIN_SPOKEN_GAIN, MAX_SPOKEN_GAIN, DEFAULT_SPOKEN_GAIN);
}

function spotifyPercentFromGain(value = S?.spotifyGain) {
  const gain = spotifyGainValue(value);
  if (gain <= MIN_SPOTIFY_GAIN) return DEFAULT_SPOTIFY_VOLUME;
  const span = Math.max(1, MAX_SPOTIFY_GAIN - MIN_SPOTIFY_GAIN);
  const t = Math.max(0, Math.min(1, (gain - MIN_SPOTIFY_GAIN) / span));
  return spotifyVolumePercent(Math.round(DEFAULT_SPOTIFY_VOLUME + Math.pow(t, 1.85) * (SPOTIFY_VOLUME_SLIDER_MAX - DEFAULT_SPOTIFY_VOLUME)));
}

function spotifyGainFromPercent(value = S?.spotifyVolume) {
  const percent = spotifyVolumePercent(value);
  if (percent <= DEFAULT_SPOTIFY_VOLUME) return MIN_SPOTIFY_GAIN;
  const span = Math.max(1, SPOTIFY_VOLUME_SLIDER_MAX - DEFAULT_SPOTIFY_VOLUME);
  const t = Math.max(0, Math.min(1, (percent - DEFAULT_SPOTIFY_VOLUME) / span));
  return spotifyGainValue(Math.round(MIN_SPOTIFY_GAIN + Math.pow(t, 1 / 1.85) * (MAX_SPOTIFY_GAIN - MIN_SPOTIFY_GAIN)));
}

function announcementGainFromSpokenGain(value = S?.spokenGain) {
  const t = spokenGainValue(value) / Math.max(1, MAX_SPOKEN_GAIN);
  return Math.round((8 + t * (MAX_ANNOUNCEMENT_GAIN - 8)) * 100) / 100;
}

function spokenGainFromAnnouncementGain(value = S?.announcementGain) {
  const level = clampNumber(value, 1, MAX_ANNOUNCEMENT_GAIN, DEFAULT_ANNOUNCEMENT_GAIN);
  const t = Math.max(0, Math.min(1, (level - 8) / Math.max(1, MAX_ANNOUNCEMENT_GAIN - 8)));
  return spokenGainValue(Math.round(t * MAX_SPOKEN_GAIN));
}
...
function voiceLoudnessLabel(value = S?.announcementGain) {
  const level = clampNumber(value, 1, MAX_ANNOUNCEMENT_GAIN, DEFAULT_ANNOUNCEMENT_GAIN);
  if (level >= MAX_ANNOUNCEMENT_GAIN) return 'PA max+';
  return `PA ${Math.round((level / MAX_ANNOUNCEMENT_GAIN) * 100)}%`;
}

function spotifyGainLabel(value = S?.spotifyGain) {
  const gain = spotifyGainValue(value);
  return `${gain} (${spotifyPercentFromGain(gain)}%)`;
}

function spokenGainLabel(value = S?.spokenGain) {
  const gain = spokenGainValue(value);
  return `+${gain} (${voiceLoudnessLabel(announcementGainFromSpokenGain(gain))})`;
}
...
function clearStaleReceiverFailures(state) {
  ...
  const hadStaleReceiverFailure = [
    state.setupNotice,
    state.lastError,
    state.spotifyLastError,
    state.spotifyStatus,
    state.feedback
  ].some(hasStaleReceiverFailure);
...
function normalize(raw) {
  ...
  const importedFromV22 = String(source.version || '') === VERSION || source.v22DefaultsApplied || source.v20DefaultsApplied;
  const importedFromRecentPoolside = importedFromV22 || source.v18DefaultsApplied;

  // V22 uses Spotify as the continuous bed. Suno is a cue/announcement source,
  ...
  const hasStoredSpokenGain = Number.isFinite(Number(source.spokenGain));
  s.spotifyGain = spotifyGainValue(hasStoredSpotifyGain ? s.spotifyGain : spotifyGainFromPercent(s.spotifyVolume));
  s.spotifyVolume = spotifyVolumePercent(hasStoredSpotifyGain ? spotifyPercentFromGain(s.spotifyGain) : s.spotifyVolume);
  s.spotifyDuckedVolume = clampNumber(s.spotifyDuckedVolume, 0, SPOTIFY_VOLUME_SLIDER_MAX, DEFAULT_SPOTIFY_DUCKED_VOLUME);
  s.spokenGain = spokenGainValue(hasStoredSpokenGain ? s.spokenGain : spokenGainFromAnnouncementGain(s.announcementGain));
  s.announcementGain = clampNumber(hasStoredSpokenGain ? announcementGainFromSpokenGain(s.spokenGain) : s.announcementGain, 1, MAX_ANNOUNCEMENT_GAIN, DEFAULT_ANNOUNCEMENT_GAIN);
  s.sunoVolume = sunoVolumePercent(s.sunoVolume);
  s.sunoDuckedVolume = clampNumber(s.sunoDuckedVolume, 0, 20, 2);
  if (source.v22VolumeDefaultsApplied !== V22_VOLUME_DEFAULTS_ID) {
    s.spotifyGain = DEFAULT_SPOTIFY_GAIN;
    s.spotifyVolume = DEFAULT_SPOTIFY_VOLUME;
    s.spotifyDuckedVolume = DEFAULT_SPOTIFY_DUCKED_VOLUME;
    s.spokenGain = DEFAULT_SPOKEN_GAIN;
    s.sunoVolume = DEFAULT_SUNO_VOLUME;
    s.announcementGain = announcementGainFromSpokenGain(s.spokenGain);
    s.spotifyDeviceId = '';
    s.spotifyDeviceName = '';
    s.spotifyReceiverReadyAt = 0;
    s.spotifyNeedsTap = true;
    s.iosVolumeBridgeStatus = `V22 clear PA voice mode: ${IOS_VOLUME_BRIDGE_FIX_TEXT}`;
    ...
    s.setupNotice = '';
    s.spotifyLastError = '';
    s.v22VolumeDefaultsApplied = V22_VOLUME_DEFAULTS_ID;
    s.v20VolumeDefaultsApplied = 'migrated-to-v22';
  }
  ...
  s.iosVolumeBridgeStatus = String(s.iosVolumeBridgeStatus || '');
  s.iosVolumeBridgeLastTarget = String(s.iosVolumeBridgeLastTarget || '');
  s.iosVolumeBridgeLastAt = Math.max(0, Number(s.iosVolumeBridgeLastAt || 0) || 0);
  ...
  s.v22DefaultsApplied = true;
  s.v20DefaultsApplied = true;
...
function markRangeEditing(ms = 1800) {
  rangeEditingUntil = Math.max(rangeEditingUntil, Date.now() + ms);
}

function rangeEditingActive() {
  return Date.now() < rangeEditingUntil;
}

function uiIsEditing() {
  if (rangeEditingActive()) return true;
...
function audioDraftControls() {
  return [
    ...
    ['spotifyGain', 'spotifyGain', MIN_SPOTIFY_GAIN, MAX_SPOTIFY_GAIN, DEFAULT_SPOTIFY_GAIN],
    ['spotifyDuckedVolume', 'spotifyDuckedVolume', 0, SPOTIFY_VOLUME_SLIDER_MAX, DEFAULT_SPOTIFY_DUCKED_VOLUME],
    ['spokenGain', 'spokenGain', MIN_SPOKEN_GAIN, MAX_SPOKEN_GAIN, DEFAULT_SPOKEN_GAIN],
...
function applyFormFieldDraft(id, value) {
  ...
  else if (id === 'iosVolumeBridgeEnabled') setIOSVolumeBridgeEnabled(text === 'true');
  else if (id === 'iosVolumeBridgeName') setIOSVolumeBridgeName(text);
...
function renderWhenIdle() {
  if (S.screen !== 'home' && uiIsEditing()) {
    pendingRender = true;
    if (rangeEditingActive() && !pendingRenderTimer) {
      const delay = Math.max(100, rangeEditingUntil - Date.now() + 90);
      pendingRenderTimer = setTimeout(() => {
        pendingRenderTimer = null;
        if (!pendingRender || uiIsEditing()) return;
        pendingRender = false;
        render();
      }, delay);
    }
...
function preserveLocalBeforeMerge() {
  return {
    ...
    iosVolumeBridgeStatus: S.iosVolumeBridgeStatus,
    iosVolumeBridgeLastTarget: S.iosVolumeBridgeLastTarget,
    iosVolumeBridgeLastAt: S.iosVolumeBridgeLastAt,
...
function shouldRetryActionNeededEvent(event, message = '') {
  if (!isRetryableMusicCommand(event)) return false;
  return false;
}
...
function beginReceiverSession(reason = 'receiver start') {
  ...
  logEvent('receiver', 'Fresh receiver session', `${reason}; stale commands ignored. Send fresh music commands after the receiver is ready.`);
  localSave();
  return startedAt;
}
...
async function processEvent(event) {
  ...
  try {
    ...
    if (isActionNeeded(error)) {
      const retryableMusicCommand = shouldRetryActionNeededEvent(event, message);
      ...
      const next = retryableMusicCommand
        ? `${message} Receiver needs a fresh command after it is ready.`
    ...
    } else if (event?.kind === 'command' && SPOTIFY_DEVICE_COMMANDS.has(event.type) && spotifyDeviceNotFound(message)) {
      markHandled(event.id);
      delete retryAfter[event.id];
      resetSpotifyReceiverDevice(message);
      const next = staleSpotifyDeviceActionMessage(message);
      logEvent('receiver', 'Spotify receiver device expired', next, { eventId: event.id, commandType: event.type });
      setActionNeeded(next);
    } else if (isRetryableMusicCommand(event) && (STALE_RECEIVER_FAILURE_PATTERN.test(message) || spotifyRestrictionLike(message))) {
      markHandled(event.id);
      delete retryAfter[event.id];
...
async function runCommand(command) {
  ...
  if (command.type === 'spotify-volume') {
    if (Number.isFinite(Number(command.spotifyGain))) {
      S.spotifyGain = spotifyGainValue(command.spotifyGain);
      S.spotifyVolume = spotifyPercentFromGain(S.spotifyGain);
    } else {
      S.spotifyVolume = spotifyVolumePercent(command.volume, S.spotifyVolume);
      S.spotifyGain = spotifyGainFromPercent(S.spotifyVolume);
    }
  } else if (command.type === 'audio-settings') {
    if (Number.isFinite(Number(command.spotifyGain))) {
      S.spotifyGain = spotifyGainValue(command.spotifyGain);
      S.spotifyVolume = spotifyPercentFromGain(S.spotifyGain);
    } else if (Number.isFinite(Number(command.spotifyVolume))) {
      S.spotifyVolume = spotifyVolumePercent(command.spotifyVolume, S.spotifyVolume);
      S.spotifyGain = spotifyGainFromPercent(S.spotifyVolume);
    }
    ...
    if (Number.isFinite(Number(command.spokenGain))) {
      S.spokenGain = spokenGainValue(command.spokenGain);
      S.announcementGain = announcementGainFromSpokenGain(S.spokenGain);
    }
    if (Number.isFinite(Number(command.sunoVolume))) S.sunoVolume = sunoVolumePercent(command.sunoVolume, S.sunoVolume);
    if (Number.isFinite(Number(command.announcementGain)) && !Number.isFinite(Number(command.spokenGain))) {
  ...
  if (command.type === 'spotify-play') {
    ...
    await setReceiverHardwareVolume('music', 'remote volume command');
    ...
    logEvent(
      'spotify',
      iosLimited ? 'Spotify volume sent; loud voice takeover active' : 'Spotify volume verified on receiver',
    ...
    else throw Error('V22 Suno cues require a pasted Suno or direct audio URL.');
...
function readMusicSettings() {
  ...
  if ($('spotifyGain')) {
    S.spotifyGain = spotifyGainValue(val('spotifyGain'));
    S.spotifyVolume = spotifyPercentFromGain(S.spotifyGain);
  } else {
  ...
  if ($('spokenGain')) {
    S.spokenGain = spokenGainValue(val('spokenGain'));
    S.announcementGain = announcementGainFromSpokenGain(S.spokenGain);
  } else {
...
function receiverAudioReady() {
  const status = typeof window.__poolsideV22AudioStatus === 'function' ? window.__poolsideV22AudioStatus() : null;
...
async function ensureReceiverAudio(reason = 'receiver action', options = {}) {
  ...
  const attempts = [];
  if (typeof window.__poolsideV22UnlockAudio === 'function') attempts.push(window.__poolsideV22UnlockAudio(reason, unlockOptions));
  if (unlockOptions.audible || typeof window.__poolsideV22UnlockAudio !== 'function') attempts.push(fallbackUnlockReceiverAudio(reason, unlockOptions));
  if (attempts.length) {
...
async function stopReceiverForegroundAudio(reason = 'stop command') {
  ...
  if (typeof window.__poolsideV22StopAudio === 'function') {
    try { stopped = !!window.__poolsideV22StopAudio(reason) || stopped; } catch {}
  }
  if (!announcementMusic.paused) {
    announcementMusic.pause();
    try { announcementMusic.currentTime = 0; } catch {}
    stopped = true;
  }
  if (!music.paused) {
    await fade(music, 0, 180).catch(() => {});
    music.pause();
  ...
  if (playback) {
    activeForegroundPlayback = null;
    await restoreSpotifyAfterAnnouncement(playback.spotifySnapshot);
    S.activeMusicProvider = 'spotify';
    S.activeMusicUrl = S.spotifyUrl || DEFAULT_SPOTIFY_PLAYLIST;
    S.activeMusicLabel = `Spotify bed restored after stopping ${playback.title || 'foreground audio'}`;
    setFeedback(`${playback.title || 'Foreground audio'} stopped; Spotify bed restored.`, true);
    localSave();
  }
...
async function stopSuno(push = true) {
  ...
  const stoppedForeground = await stopReceiverForegroundAudio('Stop Suno command');
  await fade(music, 0, 260).catch(() => {});
  music.pause();
  try { music.currentTime = 0; } catch {}
  music.volume = musicGain();
  S.intent = stoppedForeground ? 'playing' : 'stopped';
  if (!stoppedForeground) setManualMusicHold('Suno stopped manually');
  logEvent('stop', stoppedForeground ? 'Foreground Suno stopped' : 'Suno stopped', '');
  setFeedback(stoppedForeground ? 'Foreground Suno audio stopped on receiver.' : 'Suno stopped on receiver.', true);
...
async function stopSelected(push = true) {
  S.musicProvider = 'spotify';
  if (S.screen === 'home') {
    const stoppedForeground = await stopReceiverForegroundAudio('Stop command');
    if (stoppedForeground) {
      logEvent('stop', 'Foreground audio stopped', 'Stop command canceled active Suno/announcement audio.');
      await pushState('Receiver foreground stop logged.', { render: false });
      renderWhenIdle();
      return;
    }
  }
...
function readSpotifyVolumeControl() {
  const gain = $('spotifyGain');
  if (gain) {
    S.spotifyGain = spotifyGainValue(gain.value);
    S.spotifyVolume = spotifyPercentFromGain(S.spotifyGain);
  } else {
...
function updateSpotifyVolumeDraft(value) {
  ...
  if (S.screen === 'home') setReceiverHardwareVolume('music', 'Spotify slider preview').catch(() => {});
  return S.spotifyVolume;
}
...
function syncVolumeInputLimits() {
  ...
  setRangeLimits('spotifyGain', MIN_SPOTIFY_GAIN, MAX_SPOTIFY_GAIN);
  setRangeLimits('spotifyDuckedVolume', 0, SPOTIFY_VOLUME_SLIDER_MAX);
  setRangeLimits('spokenGain', MIN_SPOKEN_GAIN, MAX_SPOKEN_GAIN);
...
function syncVolumeReadouts() {
  ...
  const spotifyGain = String(spotifyGainValue(S.spotifyGain));
  const spotifyGainInput = $('spotifyGain');
  if (spotifyGainInput && String(spotifyGainInput.value) !== spotifyGain) spotifyGainInput.value = spotifyGain;
  const spotifyGainOut = $('spotifyGainOut');
  if (spotifyGainOut) spotifyGainOut.textContent = spotifyGainLabel(S.spotifyGain);
  const duckedValue = String(clampNumber(S.spotifyDuckedVolume, 0, SPOTIFY_VOLUME_SLIDER_MAX, DEFAULT_SPOTIFY_DUCKED_VOLUME));
  const duckedInput = $('spotifyDuckedVolume');
  ...
  if (suno) suno.textContent = `${sunoValue}%`;
  const gainValue = String(clampNumber(S.announcementGain, 1, MAX_ANNOUNCEMENT_GAIN, DEFAULT_ANNOUNCEMENT_GAIN));
  const gainInput = $('announcementGain');
  if (gainInput && String(gainInput.value) !== gainValue) gainInput.value = gainValue;
  const gain = $('announcementGainOut');
  if (gain) gain.textContent = voiceLoudnessLabel(gainValue);
  const spokenGain = String(spokenGainValue(S.spokenGain));
  const spokenGainInput = $('spokenGain');
  if (spokenGainInput && String(spokenGainInput.value) !== spokenGain) spokenGainInput.value = spokenGain;
  const spokenGainOut = $('spokenGainOut');
  if (spokenGainOut) spokenGainOut.textContent = spokenGainLabel(S.spokenGain);
  const rateValue = String(clampNumber(S.rate, .75, 1.15, .94));
  const rateInput = $('rate');
  if (rateInput && String(rateInput.value) !== rateValue) rateInput.value = rateValue;
  const rate = $('rateOut');
  if (rate) rate.textContent = rateValue;
  const pitchValue = String(clampNumber(S.pitch, .85, 1.15, 1));
  const pitchInput = $('pitch');
  if (pitchInput && String(pitchInput.value) !== pitchValue) pitchInput.value = pitchValue;
  const pitch = $('pitchOut');
  if (pitch) pitch.textContent = pitchValue;
}
...
function updateRangeDraft(key, value) {
  ...
  else if (key === 'spotifyGain') {
    S.spotifyGain = spotifyGainValue(value);
    S.spotifyVolume = spotifyPercentFromGain(S.spotifyGain);
  }
  ...
  else if (key === 'spokenGain') {
    S.spokenGain = spokenGainValue(value);
    S.announcementGain = announcementGainFromSpokenGain(S.spokenGain);
  }
  else if (key === 'announcementGain') {
    S.announcementGain = clampNumber(value, 1, MAX_ANNOUNCEMENT_GAIN, DEFAULT_ANNOUNCEMENT_GAIN);
    S.spokenGain = spokenGainFromAnnouncementGain(S.announcementGain);
  }
...
function audioSettingsPayload() {
  return {
    spotifyVolume: spotifyVolumePercent(S.spotifyVolume),
    spotifyGain: spotifyGainValue(S.spotifyGain),
    spotifyDuckedVolume: clampNumber(S.spotifyDuckedVolume, 0, SPOTIFY_VOLUME_SLIDER_MAX, DEFAULT_SPOTIFY_DUCKED_VOLUME),
    spokenGain: spokenGainValue(S.spokenGain),
...
function audioSettingsDetail() {
  ...
  return `Spotify gain ${audio.spotifyGain} (${audio.spotifyVolume}%); Suno ${audio.sunoVolume}%; spoken gain +${audio.spokenGain} (${voiceLoudnessLabel(audio.announcementGain)}); speed ${audio.rate}; pitch ${audio.pitch}`;
}

function applyReceiverAudioSettings(reason = 'local setting', options = {}) {
  const audio = audioSettingsPayload();
  S.spotifyVolume = audio.spotifyVolume;
  S.spotifyGain = audio.spotifyGain;
  S.spotifyDuckedVolume = audio.spotifyDuckedVolume;
  S.spokenGain = audio.spokenGain;
  ...
  if (!announcementMusic.paused) announcementMusic.volume = musicGain(audio.sunoVolume);
  if (options.log !== false) logEvent('settings', 'Audio settings applied', `${reason}: ${audioSettingsDetail()}`);
  if (options.save !== false) localSave();
}
...
async function sendSpotifyVolumeCommand(volume, options = {}) {
  ...
  const safeGain = spotifyGainValue(S.spotifyGain);
  const event = appendEvent('command', {
    type: 'spotify-volume',
    volume: safeVolume,
    spotifyGain: safeGain,
    live: !!options.live,
    label: `${options.live ? 'Preview' : 'Set'} Spotify gain ${safeGain}`,
    detail: `Spotify gain ${safeGain} maps to ${safeVolume}%; spoken commands pause music and play through the max clear PA voice path.`
  });
  if (!options.live) {
    logEvent('command', 'Set Spotify volume', `${safeVolume}% / gain ${safeGain}`, { commandType: 'spotify-volume', eventId: event.id });
  }
...
async function applySpotifyVolume(push = true, options = {}) {
  ...
  if (S.screen !== 'home' && push) {
    await sendSpotifyVolumeCommand(volume, {
      live: !!options.live,
      message: options.silent ? '' : `Spotify gain ${S.spotifyGain} (${volume}%) sent to all active receivers. Announcements stay loud.`
    });
    if (!options.silent) {
      setSpotifyStatus(`Spotify gain ${S.spotifyGain} (${volume}%) sent to receivers. Home pauses music during spoken announcements so voice can take over.`, true);
      renderWhenIdle();
    }
    return;
  }
  await setReceiverHardwareVolume('music', options.live ? 'live local volume slider' : 'local volume control');
  ...
  if (!options.silent) {
    const iosLimited = isIOSLikeBrowser() && !result?.verified;
    logEvent('spotify', iosLimited ? 'Spotify volume sent; loud voice takeover active' : 'Spotify volume set locally', `${volume}% via ${result.method || 'receiver'}${result.verified ? `; verified ${result.actualVolume}%` : ''}${result.detail ? `; ${result.detail}` : ''}`);
...
function spotifyIOSVolumeLimitDetail(volume = S.spotifyVolume) {
  return `Spotify receives ${spotifyVolumePercent(volume)}%, but iOS may keep the physical speaker loudness under hardware control. ${IOS_VOLUME_BRIDGE_FIX_TEXT}`;
}

function iosVolumeBridgeName() {
  return storageGet(IOS_VOLUME_BRIDGE_NAME_KEY) || IOS_VOLUME_BRIDGE_DEFAULT_NAME;
}

function iosVolumeBridgeEnabled() {
  return storageGet(IOS_VOLUME_BRIDGE_KEY) === 'true';
}

function setIOSVolumeBridgeEnabled(enabled) {
  storageSet(IOS_VOLUME_BRIDGE_KEY, enabled ? 'true' : 'false');
}

function setIOSVolumeBridgeName(name) {
  storageSet(IOS_VOLUME_BRIDGE_NAME_KEY, String(name || IOS_VOLUME_BRIDGE_DEFAULT_NAME).trim() || IOS_VOLUME_BRIDGE_DEFAULT_NAME);
}

function applyIOSVolumeBridgeDefault() {
...
function iosHardwareVolumeAvailable() {
  return S.screen === 'home' && isIOSLikeBrowser();
}
...
function iosVolumeBridgeTarget(kind = 'music') {
  if (kind === 'voice') return { percent: voiceHardwareVolumePercent(), label: 'voice' };
  if (kind === 'suno') return { percent: sunoHardwareVolumePercent(), label: 'Suno' };
...
async function requestIOSHardwareVolume(kind = 'music', reason = 'optional receiver volume shortcut', options = {}) {
  ...
  try {
    ...
    frame.title = 'Poolside Pulse optional iPhone volume shortcut';
    ...
    if (options.log !== false) logEvent('receiver', 'Optional iPhone shortcut requested', `${percent}% for ${target.label}; ${reason}; ${IOS_VOLUME_BRIDGE_FIX_TEXT}`);
    return true;
  } catch (error) {
    S.iosVolumeBridgeStatus = `Optional iPhone shortcut failed: ${error.message || error}`;
    localSave();
    if (options.log !== false) logEvent('receiver', 'Optional iPhone shortcut failed', S.iosVolumeBridgeStatus);
    return false;
  }
}
...
async function testIOSVolumeBridge(kind = 'music') {
  const name = val('iosVolumeBridgeName').trim() || iosVolumeBridgeName();
  setIOSVolumeBridgeName(name);
  setIOSVolumeBridgeEnabled(true);
  const target = iosVolumeBridgeTarget(kind);
  S.iosVolumeBridgeStatus = `Testing optional iPhone shortcut: ${target.percent}% for ${target.label}. ${IOS_VOLUME_BRIDGE_FIX_TEXT}`;
  logEvent('receiver', 'Optional iPhone shortcut test', `${target.percent}% for ${target.label}; shortcut ${name}; ${IOS_VOLUME_BRIDGE_FIX_TEXT}`);
  localSave();
  renderWhenIdle();
...
async function setupLoudVoiceReceiver() {
  if (S.screen !== 'home') throw Error('Loud Voice Setup must run on the speaker-connected Home receiver.');
  setIOSVolumeBridgeEnabled(false);
  storageSet(IOS_VOLUME_BRIDGE_MODE_KEY, IOS_VOLUME_BRIDGE_MODE_ID);
  S.musicProvider = 'spotify';
  S.spotifyGain = DEFAULT_SPOTIFY_GAIN;
  S.spotifyVolume = spotifyPercentFromGain(S.spotifyGain);
  S.sunoVolume = sunoVolumePercent(S.sunoVolume || DEFAULT_SUNO_VOLUME);
  S.spotifyDuckedVolume = DEFAULT_SPOTIFY_DUCKED_VOLUME;
  S.spokenGain = DEFAULT_SPOKEN_GAIN;
  S.announcementGain = announcementGainFromSpokenGain(S.spokenGain);
  S.voiceMode = 'ai';
  S.iosVolumeBridgeStatus = `V22 Loud Voice Setup active. ${IOS_VOLUME_BRIDGE_FIX_TEXT}`;
  ...
  let spotifyMessage = 'Spotify is not logged in on this receiver yet.';
  if (spotifyLoggedIn()) {
    try {
      await beginSpotifyTapActivation();
      await playSpotifyUrl(S.spotifyUrl || DEFAULT_SPOTIFY_PLAYLIST, false, { fromTap: true });
      spotifyMessage = 'Spotify bed started on this receiver.';
    } catch (error) {
      spotifyMessage = `Spotify still needs receiver attention: ${error.message || error}`;
    }
  }
  logEvent('receiver', 'Loud Voice Setup active', `${spotifyMessage} Spoken commands pause music, play through the clear PA voice path, then restore music.`);
  setFeedback(`Loud Voice Setup active. ${spotifyMessage}`, true);
...
function registerSpotifyListeners() {
  spotifyPlayer.addListener('ready', ({ device_id }) => {
    ...
    S.spotifyDeviceName = 'Poolside Pulse V22 Receiver';
...
async function primeSpotifyPlayer() {
  ...
  spotifyPrimePromise = (async () => {
    ...
    spotifyPlayer = new Spotify.Player({
      name: 'Poolside Pulse V22 Receiver',
...
async function startSpotifyReceiver({ fromTap = false } = {}) {
  ...
  S.spotifyDeviceName = S.spotifyDeviceName || 'Poolside Pulse V22 Receiver';
  S.spotifyReceiverReadyAt = Date.now();
...
async function pauseSpotifyForAnnouncement(deviceId) {
  ...
  addCandidate(savedReceiverDeviceId());
  ...
  if (spotifyLoggedIn()) {
    ...
    try { addCandidate(await spotifyPreferredDeviceId({ preferKnown: true, preferPoolside: true, allowActiveFallback: true })); } catch (error) { lastError = lastError || error.message || String(error); }
    for (const target of candidates) {
      try {
        await spotifyApi('PUT', '/me/player/pause', null, { device_id: target });
        methods.push(`Spotify API ${target === spotifyWebDeviceId ? 'receiver' : 'device'}`);
        break;
      } catch (error) {
        lastError = error.message || String(error);
      }
    ...
    try {
      ...
      methods.push('Spotify API active device');
    } catch (error) {
      lastError = lastError || error.message || String(error);
    }
  }
  if (spotifyPlayer && typeof spotifyPlayer.pause === 'function') {
    try {
      ...
      methods.push('receiver SDK');
    } catch (error) {
      lastError = lastError || error.message || String(error);
    }
  }
  if (methods.length) return [...new Set(methods)].join(' + ');
  if (lastError) logEvent('spotify', 'Spotify pause fallback failed', lastError);
  return '';
}

async function silenceSpotifyForVoice(deviceId, reason = 'voice takeover') {
  if (S.musicProvider !== 'spotify' && !S.spotifyDeviceId && !spotifyPlayer) return '';
  const methods = [];
  const targets = [];
  const addTarget = id => {
    const value = String(id || '').trim();
  ...
  addTarget(deviceId);
  addTarget(savedReceiverDeviceId());
  addTarget(spotifyWebDeviceId);
  addTarget(S.spotifyDeviceId);
  if (spotifyLoggedIn()) {
    try { addTarget((await spotifyPlaybackState())?.deviceId); } catch {}
    try { addTarget(await spotifyPreferredDeviceId({ preferKnown: true, preferPoolside: true, allowActiveFallback: true })); } catch {}
  }
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      await spotifySetLocalPlayerVolume(SPOTIFY_SILENCE_VOLUME, `${reason} local silence`);
      methods.push('receiver SDK 0%');
    } catch {}
    for (const target of targets) {
      try {
        await spotifyApi('PUT', '/me/player/volume', null, { volume_percent: SPOTIFY_SILENCE_VOLUME, device_id: target });
        methods.push(`Spotify API 0% ${target === spotifyWebDeviceId ? 'receiver' : 'device'}`);
        break;
      } catch {}
    }
    if (spotifyLoggedIn()) {
      try {
        await spotifyApi('PUT', '/me/player/volume', null, { volume_percent: SPOTIFY_SILENCE_VOLUME });
        methods.push('active Spotify API 0%');
      } catch {}
    }
    if (attempt === 0) await wait(SPOTIFY_SILENCE_REPEAT_MS);
  }
  if (methods.length) {
    const detail = [...new Set(methods)].join(' + ');
    logEvent('spotify', 'Spotify silenced before voice', detail);
    return detail;
  }
  return '';
}
...
async function resumeSpotifyAfterAnnouncement(deviceId) {
  ...
  addCandidate(spotifyWebDeviceId);
  if (spotifyLoggedIn()) {
    try { addCandidate(await spotifyPreferredDeviceId({ preferKnown: true, preferPoolside: true, allowActiveFallback: true })); } catch (error) { lastError = error.message || String(error); }
    for (const target of candidates) {
      try {
        await spotifyApi('PUT', '/me/player/play', null, { device_id: target });
        methods.push(`Spotify API ${target === spotifyWebDeviceId ? 'receiver' : 'device'}`);
        break;
      } catch (error) {
        lastError = error.message || String(error);
...
async function spotifySetVolume(percent, targetDeviceId = '', options = {}) {
  const volume = Math.max(0, Math.min(SPOTIFY_VOLUME_SLIDER_MAX, Math.round(Number(percent) || 0)));
  if (options.persist !== false) {
    S.spotifyVolume = volume;
    S.spotifyGain = spotifyGainFromPercent(volume);
    localSave();
  }
...
async function holdSpotifyBedVolume(reason = 'receiver volume hold') {
  ...
  try {
    ...
    if (now - spotifyVolumeWatchLastLogAt > SPOTIFY_VOLUME_WATCH_LOG_MS) {
      ...
      logEvent('spotify', 'Spotify volume hold active', `${S.spotifyVolume}%${result?.method ? ` via ${result.method}` : ''}${isIOSLikeBrowser() ? '; V22 pauses music during voice and restores it after' : ''}`);
...
function stopSpotifyDuckHold() {
  ...
  spotifyDuckHoldTimer = null;
}

function startSpotifyDuckHold(deviceId = '', targetVolume = DEFAULT_SPOTIFY_DUCKED_VOLUME) {
  stopSpotifyDuckHold();
  const holdVolume = clampNumber(targetVolume, 0, SPOTIFY_VOLUME_SLIDER_MAX, DEFAULT_SPOTIFY_DUCKED_VOLUME);
  spotifyDuckHoldTimer = setInterval(() => {
...
function spotifyRestrictionLike(message) {
  return /restriction violated|not allowed|not active|no active device|device not found|autoplay|platform|denied|rate|HTTP 429/i.test(String(message || ''));
}

function resetSpotifyReceiverDevice(reason = 'Spotify receiver device reset') {
  ...
  S.spotifyReceiverReadyAt = 0;
  S.spotifyNeedsTap = true;
  S.receiverStatus = 'Spotify receiver needs a fresh start.';
  S.spotifyStatus = 'Spotify receiver device expired. Tap Start Receiver + Play Spotify on this speaker phone.';
  if (detail) S.spotifyLastError = detail;
  logEvent('spotify', 'Spotify receiver device reset', detail || 'Cleared stale Spotify device ID.');
  localSave();
}

function staleSpotifyDeviceActionMessage(message = '') {
  return `Spotify lost the speaker-phone receiver device (${message || 'Device not found'}). Tap Start Receiver + Play Spotify once on the speaker-connected Home phone, then send the command again.`;
}
...
async function sendSpotifyPlayback(playUrl, deviceId) {
  ...
  const errors = [];
  await setReceiverHardwareVolume('music', 'starting Spotify playback');
  ...
  if (spotifyDeviceNotFound(detail)) {
    throw Error(detail || 'Device not found');
  }
  if (spotifyRestrictionLike(detail)) {
    throw actionNeededError('Spotify is ready but still paused because Spotify blocked remote play. On the speaker-connected Home phone, tap Start Receiver + Play Spotify once. After it starts, send Play Spotify again if needed.');
  }
  throw Error(detail || 'Spotify did not report active playback on the receiver.');
}
...
function markSpotifyPlaybackAccepted(playUrl) {
  ...
  setReceiverHardwareVolume('music', 'Spotify playback accepted').catch(() => {});
...
async function playSpotifyUrl(url = S.spotifyUrl, push = true, options = {}) {
  ...
  if (remotePreferredId) {
    try {
      ...
      if (spotifyDeviceNotFound(error.message || error)) {
        resetSpotifyReceiverDevice(error.message || String(error));
      }
    }
  }
  ...
  try {
    ...
    if (spotifyDeviceNotFound(error.message || error)) {
      resetSpotifyReceiverDevice(error.message || String(error));
      try {
        const freshDeviceId = await startSpotifyReceiver({ fromTap: !!options.fromTap });
        if (options.fromTap) await activateSpotifyElement();
        audible = await sendSpotifyPlayback(playUrl, freshDeviceId);
      } catch (retryError) {
        if (spotifyDeviceNotFound(retryError.message || retryError)) {
          throw actionNeededError(staleSpotifyDeviceActionMessage(retryError.message || retryError));
        }
        throw retryError;
      }
    } else if (options.fromTap && isIOSLikeBrowser() && !spotifyFreshTapActivatedPlayer && spotifyRestrictionLike(error.message)) {
...
async function spotifyPause(push = true) {
  ...
  try {
    ...
    if (spotifyDeviceNotFound(error.message || error)) {
      resetSpotifyReceiverDevice(error.message || String(error));
      throw actionNeededError(staleSpotifyDeviceActionMessage(error.message || error));
    }
    throw error;
  }
  S.intent = 'paused';
...
async function spotifyNext(push = true) {
  ...
  S.intent = 'playing';
  await setReceiverHardwareVolume('music', 'Spotify skip');
...
async function duckSpotifyForAnnouncement(options = {}) {
  ...
  const current = await spotifyCurrentPlaybackDevice({ requirePlaying: true, allowActiveFallback: true });
  const snapshot = {
    volume: current?.deviceId && Number.isFinite(Number(current.volume)) ? current.volume : spotifyVolumePercent(S.spotifyVolume),
    wasPlaying: !!current?.isPlaying || S.intent === 'playing',
    deviceId: current?.deviceId || savedReceiverDeviceId(),
    targetVolume,
    pausedForDuck: false,
    duckMethod: ''
  };
  try {
    ...
    const deviceId = snapshot.deviceId || await spotifyPreferredDeviceId({ preferKnown: true, preferPoolside: true, allowActiveFallback: true });
    ...
    if (pauseSpotify) {
      stopSpotifyVolumeWatch();
      const silenceMethod = await silenceSpotifyForVoice(snapshot.deviceId, 'pre-voice takeover');
      const pauseMethod = await pauseSpotifyForAnnouncement(snapshot.deviceId);
      if (!pauseMethod && snapshot.wasPlaying && !silenceMethod) throw Error('Spotify did not accept a silence or pause command for foreground music.');
      if (!pauseMethod && !silenceMethod) return snapshot;
      snapshot.pausedForDuck = !!pauseMethod;
      snapshot.wasPlaying = true;
      snapshot.duckMethod = [silenceMethod ? `silence via ${silenceMethod}` : '', pauseMethod ? `pause via ${pauseMethod}` : ''].filter(Boolean).join(' + ');
      logEvent('spotify', pauseMethod ? 'Spotify silenced and paused for voice takeover' : 'Spotify silenced for voice takeover', snapshot.duckMethod);
      setFeedback('Spotify is being silenced before the spoken command; it will restore afterward.', true);
      return snapshot;
    }
    let localSet = false;
    let remoteSet = false;
    ...
    if (!verified && snapshot.wasPlaying) {
      ...
      if (pauseMethod) {
        ...
        logEvent('spotify', 'Spotify paused for foreground audio', `Low-volume duck was not verified${remoteError ? ` (${remoteError})` : ''}; music will resume after foreground audio.`);
        setFeedback('Spotify could not be verified at the lowered volume, so it paused for the foreground audio and will resume after.', true);
        return snapshot;
      }
    }
    ...
    startSpotifyDuckHold(snapshot.deviceId, targetVolume);
    logEvent(verified ? 'spotify' : 'receiver', verified ? 'Spotify ducked for voice audio' : 'Spotify duck sent but unverified', `${targetVolume}% via ${snapshot.duckMethod}.`);
    setFeedback(`Spotify lowered to ${targetVolume}% for voice audio. The current song keeps running.`, true);
    return snapshot;
  } catch (error) {
...
async function reinforceSpotifySilenceForVoice(snapshot, reason = 'final voice takeover guard') {
  if (!snapshot || S.musicProvider !== 'spotify') return '';
  const methods = [];
  try {
    const silenceMethod = await silenceSpotifyForVoice(snapshot.deviceId, reason);
    if (silenceMethod) methods.push(`silence via ${silenceMethod}`);
  } catch {}
  try {
    const pauseMethod = await pauseSpotifyForAnnouncement(snapshot.deviceId);
    if (pauseMethod) {
      snapshot.pausedForDuck = true;
      snapshot.wasPlaying = true;
      methods.push(`pause via ${pauseMethod}`);
    }
  } catch {}
  if (methods.length) {
    const detail = [...new Set(methods)].join(' + ');
    snapshot.duckMethod = [snapshot.duckMethod, `final ${detail}`].filter(Boolean).join(' + ');
    logEvent('spotify', 'Spotify final voice takeover guard', detail);
    return detail;
  }
  return '';
}
...
async function restoreSpotifyAfterAnnouncement(snapshot) {
  ...
  if (!snapshot) return;
  try {
    if (snapshot.wasPlaying && !manualMusicHoldActive()) await wait(VOICE_TAKEOVER_POST_ROLL_MS);
    ...
    if (snapshot.wasPlaying && !manualMusicHoldActive()) {
      await setReceiverHardwareVolume('music', 'Spotify restore after foreground audio');
      if (snapshot.pausedForDuck) {
        ...
        if (resumeMethod) {
          ...
          startSpotifyVolumeWatch('Spotify restore after announcement');
        }
      } else {
        try {
          const player = await spotifyApi('GET', '/me/player');
...
async function playReceiverAudioUrl(url, options = {}) {
  const raw = String(url || '').trim();
  if (!raw) throw Error('No playable audio URL was provided.');
  const volume = clampNumber(options.volume, 0, 1, 1);
  const label = options.label || 'Receiver audio';
  if (typeof window.__poolsideV22PlayAudioUrl === 'function') {
    return await window.__poolsideV22PlayAudioUrl(raw, {
      label,
      volume,
      gain: volume,
      minGain: 0,
  ...
  if (typeof window.__poolsideV22PlayAnnouncementBlob === 'function') {
    try {
      const response = await fetch(raw, { cache: 'no-store' });
      if (!response.ok) throw Error(`Audio URL returned HTTP ${response.status}`);
      const blob = await response.blob();
      return await window.__poolsideV22PlayAnnouncementBlob(blob, {
        label,
        volume,
        gain: volume,
        minGain: 0,
      ...
      logEvent('receiver', `${label} Web Audio path failed`, error.message || String(error));
    }
  }
  announcementMusic.pause();
  announcementMusic.src = raw;
  announcementMusic.muted = false;
  announcementMusic.volume = volume;
...
async function performSunoUrlCue(url, options = {}) {
  ...
  try {
    ...
    S.activeMusicLabel = `Suno cue playing: ${title}`;
    setFeedback(`Suno cue started: ${title}. Use Stop to cancel it and restore Spotify.`, true);
    logEvent('play', 'Suno cue started', `${title} · ${compactUrl(raw)}`, { url: raw, eventId: options.eventId || '' });
    await setReceiverHardwareVolume('suno', 'Suno cue');
    const playbackPromise = playReceiverAudioUrl(chosen.audioUrl, { label: 'Suno cue', volume: musicGain(S.sunoVolume) })
      .then(completed => {
        if (!playback.stopped && completed !== false) {
          logEvent('play', 'Suno cue finished', `${title} · ${compactUrl(raw)}`, { url: raw, eventId: options.eventId || '' });
          setFeedback(`Suno cue finished; Spotify bed restored: ${title}.`, true);
        }
      })
      .catch(error => {
        if (!playback.stopped) {
          logEvent('receiver', 'Suno cue playback failed', error.message || String(error), { url: raw, eventId: options.eventId || '' });
          setFeedback(`Suno cue playback failed: ${error.message || error}`, false);
        }
      })
      .finally(async () => {
        if (activeForegroundPlayback?.id === playback.id) {
          activeForegroundPlayback = null;
    ...
    speaking = false;
    await pushState('Receiver Suno cue started.', { render: false });
    renderWhenIdle();
    return true;
  } catch (error) {
...
async function performSunoUrlAnnouncement(url, options = {}) {
  ...
  try {
    ...
    try {
      await setReceiverHardwareVolume('suno', 'Suno URL announcement');
      await playReceiverAudioUrl(chosen.audioUrl, { label: 'Suno announcement', volume: musicGain(S.sunoVolume) });
...
async function performSunoAnnouncement(trackIndex, options = {}) {
  ...
  try {
    ...
    const sunoSnapshot = await duckSunoForAnnouncement();
    try {
      await setReceiverHardwareVolume('suno', 'Suno announcement');
      await playReceiverAudioUrl(chosen.item.audioUrl, { label: 'Suno announcement', volume: musicGain(S.sunoVolume) });
...
async function fetchAiVoiceBlob(message) {
  const response = await fetch(API.tts, {
    ...
    body: JSON.stringify({
      ...
      instructions: 'Speak like a calm, polished resort public-address announcer at Serenity Shores. Use a projected, close-mic PA voice with strong consonants, steady level, and no background noise. Keep it clear, firm, warm, and intentionally louder than pool music without sounding distorted.'
    })
  });
...
async function playAnnouncementBlob(blob) {
  if (typeof window.__poolsideV22PlayAnnouncementBlob === 'function') {
    return await window.__poolsideV22PlayAnnouncementBlob(blob, {
      ...
      gain: clampNumber(S.announcementGain, 1, MAX_ANNOUNCEMENT_GAIN, DEFAULT_ANNOUNCEMENT_GAIN),
      voiceGain: spokenGainValue(S.spokenGain),
      preRollMs: VOICE_TAKEOVER_PRE_ROLL_MS,
      minGain: 1,
      maxGain: MAX_ANNOUNCEMENT_GAIN,
      volume: 1
    });
  }
...
async function performAnnouncement(text, options = {}) {
  ...
  const sunoSnapshot = await duckSunoForAnnouncement();
  try {
    if (spotifySnapshot?.duckMethod || spotifySnapshot?.pausedForDuck || sunoSnapshot?.wasPlaying) {
      S.audioStatus = 'V22 Max Gap voice takeover: music is silenced before speech starts.';
      await wait(VOICE_TAKEOVER_PRE_ROLL_MS);
      await reinforceSpotifySilenceForVoice(spotifySnapshot);
    }
    await setReceiverHardwareVolume('voice', 'voice announcement');
    await ensureReceiverAudio('announcement', { required: true });
    if (voiceBlob) {
      await playAnnouncementBlob(voiceBlob);
      setFeedback(`AI voice announcement completed through ${voiceLoudnessLabel()} clear PA voice.`, true);
    } else {
...
function scheduledTimeTodayMs(time) {
...
async function tick() {
  ...
  const catchupSince = scheduleBlockedSince;
  scheduleBlockedSince = 0;
  for (const item of scheduleItems(mode)) {
    if (!item.enabled) continue;
    const time = schedTime(item);
    const dueAt = scheduledTimeTodayMs(time);
    const exactDue = time === now;
    const catchupDue = catchupSince &&
      dueAt >= catchupSince - 15000 &&
      dueAt <= nowMs &&
      nowMs - dueAt <= SCHEDULE_CATCHUP_MS;
    if (!exactDue && !catchupDue) continue;
    const key = `${today()}:${mode}:${item.id}:${time}`;
    if (S.lastRun[item.id] === key) continue;
    S.lastRun[item.id] = key;
    localSave();
    if (catchupDue && !exactDue) logEvent('schedule', 'Caught up scheduled item after receiver audio', `${item.label || item.type || 'Schedule item'} · ${pretty(time)}`);
...
function statusCards() {
  ...
  return `<div class="stats"><div class="stat"><b>Music</b><strong>${esc(S.spotifyVolume)}% / gain ${esc(S.spotifyGain)}</strong></div><div class="stat"><b>Receiver</b><strong>${esc(S.screen === 'home' ? receiverReadiness() : (S.receiverStatus || 'No receiver report yet.'))}</strong></div><div class="stat"><b>Remote</b><strong>Event inbox ${inbox}</strong></div><div class="stat"><b>Voice</b><strong>${esc(spokenGainLabel())}</strong></div></div>`;
}
...
function receiverActionButtons() {
  ...
  return `<div class="receiverActions">${primary}<button id="loudVoiceSetupHome" class="secondary">Loud Voice Setup</button><button id="testVoiceHome" class="secondary">Test Voice</button><button id="testToneHome" class="secondary">Test Tone</button><button id="checkWeatherHome" class="secondary">Check Weather</button><button id="skipHome" class="secondary">Skip</button><button id="stopHome" class="secondary">Stop</button></div>`;
}
...
function iosVolumeBridgeControls() {
...
function spotifyDiagnostics() {
  ...
  return `<div class="statusBar"><b>Spotify:</b> ${esc(login)}<br><b>Status:</b> ${esc(S.spotifyStatus || 'Not checked yet.')}<br><b>${esc(product)}</b><br><b>${esc(S.spotifyDevicesSummary || 'Devices: not checked')}</b><br><b>Loud Voice:</b> ${esc(S.iosVolumeBridgeStatus || IOS_VOLUME_BRIDGE_FIX_TEXT)}<br><b>Redirect URI:</b> <code>${esc(spotifyRedirectUri())}</code>${S.spotifyLastError ? `<br><b>Last Spotify issue:</b> ${esc(S.spotifyLastError)}` : ''}</div>`;
}
  ...
  return shell(`
    <section class="panel">
      ...
      <div class="sourceBoard">
        <div class="sourceTile"><b>Spotify Bed</b><strong>${esc(sourceLabel('spotify', activeProviderUrl()))}</strong><span>${esc(S.spotifyUrl || DEFAULT_SPOTIFY_PLAYLIST)}</span></div>
        <div class="sourceTile"><b>Suno Cue</b><strong>${esc(sunoReady ? sourceLabel('suno', S.playlistUrl) : 'No saved cue')}</strong><span>${esc(sunoReady ? S.playlistUrl : 'Paste a Suno song, playlist, or direct audio URL below.')}</span></div>
        <div class="sourceTile"><b>Balance</b><strong>${esc(spotifyGainLabel())}</strong><span>Spoken gain ${esc(spokenGainLabel())}</span></div>
      </div>
      <div class="buttonStack"><button id="spotifyPlayNow">Play Spotify on Receivers</button><button id="sunoPlayNow" class="secondary">Play Saved Suno Cue</button></div>
      <div class="quickMusic"><label>Play Spotify or Suno Cue URL<input id="quickMusicUrl" value="${esc(S.quickMusicUrl || S.spotifyUrl || DEFAULT_SPOTIFY_PLAYLIST)}" placeholder="Paste Spotify, Suno, or direct audio URL"></label><div class="buttonStack"><button id="playAnyUrl">Play Pasted URL</button><button id="savePastedUrl" class="secondary">Save URL</button></div></div>
      ...
      <label>Saved Suno Cue URL<input id="playlistUrl" value="${esc(S.playlistUrl)}" placeholder="Paste a Suno song, playlist, or direct audio URL"></label>
      ...
      <div class="sectionHead"><h2>Audio Balance Manager</h2><span class="pill">V22</span></div>
      <div class="grid4">
        <label><span>Spotify Gain <output id="spotifyGainOut">${esc(spotifyGainLabel())}</output></span><input id="spotifyGain" type="range" min="${MIN_SPOTIFY_GAIN}" max="${MAX_SPOTIFY_GAIN}" step="25" value="${esc(S.spotifyGain)}"></label>
        <label><span>Spotify Volume <output id="spotifyVolumeOut">${esc(S.spotifyVolume)}%</output></span><input id="spotifyVolume" type="range" min="${SPOTIFY_VOLUME_SLIDER_MIN}" max="${SPOTIFY_VOLUME_SLIDER_MAX}" step="1" value="${esc(S.spotifyVolume)}"></label>
        ...
        <label><span>Clear PA Voice <output id="announcementGainOut">${esc(voiceLoudnessLabel())}</output></span><input id="announcementGain" type="range" min="1" max="${MAX_ANNOUNCEMENT_GAIN}" step=".05" value="${esc(S.announcementGain)}"></label>
      </div>
      <div class="grid2">
        <label><span>Suno Cue Volume <output id="sunoVolumeOut">${esc(S.sunoVolume)}%</output></span><input id="sunoVolume" type="range" min="${SUNO_VOLUME_SLIDER_MIN}" max="${SUNO_VOLUME_SLIDER_MAX}" step="1" value="${esc(S.sunoVolume)}"></label>
        <label><span>Spotify Under Voice Fallback <output id="spotifyDuckedVolumeOut">${esc(S.spotifyDuckedVolume)}%</output></span><input id="spotifyDuckedVolume" type="range" min="0" max="${SPOTIFY_VOLUME_SLIDER_MAX}" step="1" value="${esc(S.spotifyDuckedVolume)}"></label>
      </div>
      ${spotifyDiagnostics()}
      <div class="actions"><button id="spotifyLogin" class="secondary">Login with Spotify</button><button id="spotifyCheck" class="secondary">Check Spotify</button><button id="spotifyClear" class="secondary">Clear Spotify</button><button id="import" class="secondary">Check Suno URL</button></div>
    </section>`);
}
...
function render() {
  try {
    document.title = `Lake123 - Poolside Pulse - ${DISPLAY_VERSION}`;
    const app = $('app');
    ...
    app.innerHTML = S.screen === 'home'
      ...
      : !S.admin
        ? login()
        : ({ command: commandPage, music: musicPageV22, party: () => schedulePage('party'), schedule: () => schedulePage('daily'), weather: weatherPage, voice: voicePage, hours: hoursPage }[S.tab] || commandPage)();
    bind();
  } catch (error) {
    $('app').innerHTML = `<main class="wrap"><section class="panel"><h1>Poolside Pulse ${esc(DISPLAY_VERSION)}</h1><div class="alert bad"><b>Recovered from runtime error:</b> ${esc(error.message)}</div><button onclick="location.reload()">Reload app</button></section></main>`;
  }
}
...
function bind() {
  ...
  wire('testVoiceHome', () => sendAnnouncement(`This is Poolside Pulse ${DISPLAY_VERSION} loud voice test. Music should pause while this announcement plays, then resume after the voice finishes.`, false, 'Loud voice test'));
  ...
  document.querySelectorAll('#spotifyVolumeCommand, #spotifyVolume, #spotifyGain').forEach(input => {
    const beginRangeEdit = () => markRangeEditing(2500);
    input.onpointerdown = beginRangeEdit;
    input.ontouchstart = beginRangeEdit;
    input.onfocus = beginRangeEdit;
    input.oninput = () => {
      markRangeEditing(2500);
      if (input.id === 'spotifyGain') updateRangeDraft(input.id, input.value);
      else updateSpotifyVolumeDraft(input.value);
  ...
  document.querySelectorAll('#spotifyDuckedVolume, #sunoVolume, #spokenGain, #announcementGain, #rate, #pitch').forEach(input => {
    const beginRangeEdit = () => markRangeEditing(2500);
    input.onpointerdown = beginRangeEdit;
    input.ontouchstart = beginRangeEdit;
    input.onfocus = beginRangeEdit;
  ...
  wire('useSuno', async () => {
    ...
    S.musicProvider = 'spotify';
    rememberActiveSource('spotify', S.spotifyUrl, 'selected');
    await save('V22 keeps Spotify as the bed music. Paste a Suno URL and use Play Saved Suno Cue for Suno.');
  });
  ...
  wire('makeReceiver', async () => {
    readMusicSettings();
    setFeedback('Command stays Command in V22. Open Home on the speaker-connected phone, then tap Start Receiver + Play Spotify there.', true);
  });
  ...
  wire('saveVoice', async () => {
    ...
    if ($('spokenGain')) {
      ...
      S.announcementGain = clampNumber(val('announcementGain') || S.announcementGain, 1, MAX_ANNOUNCEMENT_GAIN, DEFAULT_ANNOUNCEMENT_GAIN);
      S.spokenGain = spokenGainFromAnnouncementGain(S.announcementGain);
    }
  ...
  wire('testVoice', async () => {
    ...
    S.aiVoice = val('aiVoice') || S.aiVoice;
    S.deviceVoice = val('deviceVoice');
    await sendAnnouncement(`This is Serenity Shores Poolside Pulse ${DISPLAY_VERSION}. The selected voice is being tested on the Home receiver.`, false, 'Voice test');
  });
  wire('testDevice', async () => {
    const oldMode = S.voiceMode;
    S.voiceMode = 'device';
    S.deviceVoice = val('deviceVoice');
    await sendAnnouncement(`This is the device voice fallback test for Poolside Pulse ${DISPLAY_VERSION}.`, false, 'Device voice test');
    S.voiceMode = oldMode;
    localSave();
  });
...
window.addEventListener('poolside-v22-audio-status', event => {
