import { DEFAULT_PARTY_SCHEDULE } from './v13-party-defaults.js';

const VERSION = '13';
const PIN = '7900';
const KEY = 'poolside-pulse-v13';
const DEVICE_KEY = 'poolside-pulse-v13-device-id';
const HANDLED_KEY = 'poolside-pulse-v13-handled-events';
const LOG_CLEAR_KEY = 'poolside-pulse-v13-log-cleared-at';
const RECEIVER_SESSION_KEY = 'poolside-pulse-v13-receiver-session-started-at';
const RECEIVER_ENABLED_KEY = 'poolside-pulse-v13-receiver-enabled';
const SPOTIFY_TOKEN_KEY = 'poolside-pulse-v13-spotify-token';
const APP_QUERY = '?v=13';
...
function normalize(raw) {
  ...
  if (importedFromOlderVersion && Number(source.spotifyVolume || 0) >= 90) s.spotifyVolume = DEFAULT_MUSIC_VOLUME;
  if (importedFromOlderVersion && Number(source.sunoVolume || 0) >= 90) s.sunoVolume = s.spotifyVolume;
...
function setReceiverEnabled(enabled, reason = 'receiver switch') {
  if (enabled) {
    ...
    S.receiverStatus = receiverAudioReady()
      ...
      : 'Receiver switch is on; tap Activate Receiver to unlock audio.';
...
async function runCommand(command) {
  ...
  if (command.type === 'spotify-play') {
    ...
    try {
      if (spotifyLoggedIn() && (S.activeMusicProvider === 'spotify' || S.musicProvider === 'spotify' || S.spotifyDeviceId || spotifyPlayer)) {
      ...
      method = `local audio updated; Spotify volume retry skipped: ${error.message || String(error)}`;
      logEvent('spotify', 'Spotify volume update skipped', method, { eventId: command.id, commandType: command.type });
    }
...
async function primeMusicElement(reason = 'receiver activation') {
  ...
  try {
    ...
    music.muted = true;
    music.volume = 0;
    music.src = fallbackToneUrl();
    music.load();
    await promiseTimeout(music.play(), 1000, 'Suno/music element play timed out.');
    await wait(65);
    music.pause();
    try { music.currentTime = 0; } catch {}
    if (original.src) music.src = original.src;
    else music.removeAttribute('src');
    music.muted = original.muted;
    music.volume = Number.isFinite(original.volume) ? original.volume : musicGain(S.sunoVolume);
    musicElementPrimed = true;
    fallbackAudioUnlocked = true;
    S.audioStatus = `Receiver music element primed by ${reason}.`;
    return true;
  } catch (error) {
    ...
    music.muted = original.muted;
    music.volume = Number.isFinite(original.volume) ? original.volume : musicGain(S.sunoVolume);
    S.audioStatus = `Receiver audio is active, but Suno/music needs one more receiver tap: ${error.message || String(error)}`;
    return false;
  }
}
...
function helperAudioStatus() {
  const fn = window.__poolsideV13AudioStatus || window.__poolsideV9AudioStatus;
  return typeof fn === 'function' ? fn() : null;
}

function helperUnlockAudio(reason, options) {
  const fn = window.__poolsideV13UnlockAudio || window.__poolsideV9UnlockAudio;
  return typeof fn === 'function' ? fn(reason, options) : null;
}

function helperPlayAnnouncementBlob(blob, options) {
  const fn = window.__poolsideV13PlayAnnouncementBlob || window.__poolsideV9PlayAnnouncementBlob;
...
async function ensureReceiverAudio(reason = 'receiver action', options = {}) {
  ...
  S.receiverStatus = unlocked ? 'Receiver audio ready.' : 'Receiver online; tap Activate Receiver once for sound.';
  ...
  if (!unlocked) {
    ...
    S.setupNotice = detail && !/not been activated yet/i.test(detail)
      ? detail
      : 'Tap Activate Receiver on this speaker-connected device once. iPhone browsers require that tap before music or announcements can be heard.';
  }
  localSave();
...
async function testReceiverTone(reason = 'iPhone receiver test tone') {
  ...
  try {
    ...
    if (!ok) throw actionNeededError(S.setupNotice || 'Tap Activate Receiver on this iPhone before sending sound.');
    setFeedback('Receiver audio test completed on this phone.', true);
    logEvent('receiver', 'Receiver audio test', 'Activate Receiver test tone completed on this device.');
...
async function activateLocalReceiver(options = {}) {
  ...
  if (play) {
    if (S.musicProvider === 'spotify') {
      if (!spotifyLoggedIn()) throw actionNeededError('Receiver audio is active. Tap Login Spotify on this receiver, then Activate + Play Spotify again.');
...
async function applySpotifyVolume(push = true) {
  ...
  try {
    if (spotifyLoggedIn() && (S.musicProvider === 'spotify' || S.activeMusicProvider === 'spotify' || S.spotifyDeviceId || spotifyPlayer)) {
    ...
    logEvent('spotify', 'Spotify volume local update skipped', error.message || String(error));
  }
...
async function completeSpotifyLogin() {
  ...
  try {
    ...
    setFeedback(receiverEnabled() ? 'Spotify connected on this receiver. Tap Activate + Play Spotify.' : 'Spotify connected on this browser. Command will send playback to all active receivers.', true);
  } catch (error) {
...
function registerSpotifyListeners() {
  spotifyPlayer.addListener('ready', ({ device_id }) => {
    ...
    S.spotifyDeviceName = 'Poolside Pulse V13 Receiver';
  ...
  spotifyPlayer.addListener('not_ready', () => {
    ...
    setSpotifyStatus('Spotify receiver went offline. Keep the receiver open and tap Activate + Play again.', false);
    localSave();
    renderWhenIdle();
  });
...
async function primeSpotifyPlayer() {
  ...
  spotifyPrimePromise = (async () => {
    ...
    spotifyPlayer = new Spotify.Player({
      name: 'Poolside Pulse V13 Receiver',
...
function warmSpotifyReceiver() {
  if (!receiverEnabled() || !receiverAudioReady() || S.musicProvider !== 'spotify' || !spotifyLoggedIn()) return;
...
async function startSpotifyReceiver({ fromTap = false } = {}) {
  ...
  try {
    ...
    if (current?.deviceId) {
      ...
      setSpotifyStatus(`Spotify did not start on this iPhone browser: ${error.message || error}. Keep this receiver open and tap Activate Receiver + Spotify again.`, false);
      localSave();
      throw error;
    }
  }
  S.spotifyDeviceId = deviceId;
  S.spotifyDeviceName = S.spotifyDeviceName || 'Poolside Pulse V13 Receiver';
  S.spotifyReceiverReadyAt = Date.now();
  S.receiverStatus = 'Spotify receiver active.';
  S.receiverLastSeen = stamp();
  receiverActive = true;
  ...
  if (receiverEnabled()) {
    ...
    if (spotifyLoggedIn()) {
      try {
        ...
        throw actionNeededError(`Spotify receiver is not ready on this device yet. Tap Activate Receiver + Spotify on the speaker-connected receiver. ${error.message || ''}`.trim());
      }
    }
...
async function playSpotifyUrl(url = S.spotifyUrl, push = true, options = {}) {
  ...
  if (audible) {
    ...
    const message = 'Spotify accepted the command, but this receiver is not the audible Spotify device yet. Tap Activate Receiver + Spotify on the speaker-connected receiver once.';
    S.intent = 'stopped';
    S.spotifyNeedsTap = true;
...
async function spotifyPause(push = true) {
  ...
  const method = await pauseSpotifyForAnnouncement(deviceId);
  if (!method) throw actionNeededError('Spotify is playing, but this receiver could not pause the active Spotify device. Tap Activate Receiver + Spotify once, then press Stop again.');
  S.intent = 'paused';
...
async function performSunoAnnouncement(trackIndex, options = {}) {
  ...
  try {
    ...
    throw actionNeededError(`Receiver could not start the Suno announcement: ${error.message}. Tap Activate Receiver on the speaker-connected device, then send it again.`);
  } finally {
    speaking = false;
    localSave();
    renderWhenIdle();
...
async function performAnnouncement(text, options = {}) {
  ...
  try {
    ...
    throw actionNeededError(`Receiver could not start voice audio: ${error.message}. Tap Activate Receiver on the speaker-connected device, then send Speak Now again.`);
  } finally {
    await restoreSpotifyAfterAnnouncement(spotifySnapshot);
    await restoreSunoAfterAnnouncement(sunoSnapshot);
    speaking = false;
...
function speakPartyText(text, profile) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      if (settled) return;
      settled = true;
      reject(Error('Party speech did not start. Tap Activate Receiver on the speaker-connected device.'));
    }, 5000);
...
async function runPartyCue(cueId, { test = false } = {}) {
  if (!receiverEnabled()) throw actionNeededError('Receiver is off on this device. Turn Receiver On and tap Activate Receiver before starting the party schedule.');
...
function receiverReadiness() {
  ...
  if (!receiverSessionStartedAt()) return 'Activate Receiver to begin a fresh receiver session.';
  if (!receiverAudioReady()) return 'Tap Activate Receiver on this speaker-connected device.';
  if (S.musicProvider === 'spotify') {
    ...
    if (!spotifyDeviceReady()) return 'Tap Activate + Play Spotify on this receiver.';
...
function header() {
  return `<header class="top"><div class="brand"><div class="brandMark"><span>Lake</span><b>123</b></div><div class="brandText"><b>Lake123</b><small>Poolside Pulse · Version 13</small></div></div><nav class="modeSwitch"><button id="home" class="${S.screen === 'home' ? 'on' : ''}" aria-pressed="${S.screen === 'home'}">Home</button><button id="cmd" class="${S.screen !== 'home' ? 'on' : ''}" aria-pressed="${S.screen !== 'home'}">Command</button></nav></header>`;
}
...
function readinessSteps() {
  ...
  const musicOk = S.musicProvider === 'spotify'
    ? spotifyLoggedIn() && spotifyDeviceReady()
    : hasPlayableSuno();
  const steps = [
    { number: 1, label: 'Activate Receiver', ok: receiverOk, action: 'audio', help: receiverEnabled() ? 'Tap to unlock this iPhone speaker.' : 'Tap to turn on this receiver.' },
    { number: 2, label: 'Connect Music', ok: musicOk, action: S.musicProvider === 'spotify' && !spotifyLoggedIn() ? 'login' : 'spotify', help: S.musicProvider === 'spotify' ? 'Tap to make Spotify audible here.' : 'Tap to import/play Suno.' },
...
function receiverActionButtons() {
  ...
  const needsStart = !receiverEnabled() || !audioOk || (spotifyMode && !spotifyDeviceReady());
  const label = receiverCanPause()
    ? 'Pause Music'
    : needsStart
      ? (spotifyMode ? 'Activate Receiver + Spotify' : 'Activate Receiver')
      : 'Play / Resume Music';
...
function receiverNotice() {
  let message = '';
  if (!receiverSessionStartedAt() || !receiverAudioReady()) message = 'Tap Activate Receiver. That one tap starts a fresh receiver session, ignores old commands, and unlocks iPhone audio.';
  else if (S.musicProvider === 'spotify' && !spotifyLoggedIn()) message = 'Tap Login Spotify on This Receiver. Spotify must be connected on any active receiver that should play Spotify.';
  else if (S.musicProvider === 'spotify' && !spotifyDeviceReady()) message = 'Tap Activate Receiver + Spotify or the music readiness row to make this device the speaker.';
  ...
  return partyCues({ includeDisabled: true }).map(cue => {
    ...
    return `<article class="partyCue ${current ? 'current' : ''} ${selected ? 'selected' : ''} ${cue.enabled ? '' : 'off'} ${collapsed ? 'collapsed' : ''}" tabindex="0" data-select-cue="${esc(cue.id)}">
      ...
      <div class="partyCueActions">
        <button class="secondary slim" data-collapse-cue="${esc(cue.id)}">${collapsed ? 'Spin Up' : 'Spin Down'}</button>
...
async function handleReadinessAction(action) {
  ...
  if (action === 'audio') {
    await activateLocalReceiver({ play: false, testTone: true, reason: 'readiness step' });
    return;
  }
  if (action === 'spotify') {
...
async function restorePartyDefaults() {
  if (!confirm('Restore the default Version 13 party schedule? This replaces the editable party cue list on this app.')) return;
  S.partySchedule = normalizePartySchedule(clone(DEFAULT_PARTY_SCHEDULE));
  S.partySelectedCueId = S.partySchedule.cues[0]?.id || '';
  S.partyImportText = '';
  S.partyExportText = '';
  await save('Default Version 13 party schedule restored.');
}
...
function bind() {
  ...
  wire('partyRestart', async () => {
    if (!confirm('Restart the Version 13 party schedule from the first enabled cue?')) return;
  ...
  wire('activateCmdReceiver', () => activateLocalReceiver({ play: false, testTone: true, reason: 'Command receiver activation' }));
...
function handleHelperAudioStatus(event) {
  if (event.detail?.status) {
    ...
    if (helperAudioReady(event.detail)) {
      ...
      receiverActive = false;
      S.receiverStatus = 'Receiver online; tap Activate Receiver once for sound.';
...
window.addEventListener('poolside-v13-audio-status', handleHelperAudioStatus);
