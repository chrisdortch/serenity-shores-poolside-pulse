const VERSION = '7.0';
const PIN = '7900';
const KEY = 'poolside-pulse-v7';
const DEVICE_KEY = 'poolside-pulse-v7-device-id';
const HANDLED_KEY = 'poolside-pulse-v7-handled-events';
const LOG_CLEAR_KEY = 'poolside-pulse-v7-log-cleared-at';
const SPOTIFY_TOKEN_KEY = 'poolside-pulse-v7-spotify-token';
...
function normalize(raw) {
  ...
  s.autoStop = bool(s.autoStop, false);
  s.playbackMode = ['always', 'hours'].includes(s.playbackMode) ? s.playbackMode : 'always';
  s.anns = list(s.anns).length ? list(s.anns).map(item => ({ ...item, id: String(item.id || uid()) })) : clone(DEFAULT_ANNS);
  const annIds = new Set(s.anns.map(item => item.id));
  for (const item of DEFAULT_ANNS) {
    if (!annIds.has(item.id)) s.anns.push(clone(item));
  }
...
async function sendAnnouncement(text, hold = false, label = '') {
  ...
  const event = appendEvent('announcement', { text: msg, hold: !!hold, label: label || (hold ? 'Safety announcement' : 'Announcement') });
...
async function processEvent(event) {
  ...
  try {
    ...
    logEvent('receiver', 'Event waiting on receiver', error.message || String(error), { eventId: event.id });
    setFeedback(`Receiver will retry ${event.kind || 'event'}: ${error.message || error}`, false);
...
async function ensureReceiverAudio(reason = 'receiver action') {
  if (typeof window.__poolsideV7UnlockAudio === 'function') {
    await window.__poolsideV7UnlockAudio(reason);
  }
  const status = typeof window.__poolsideV7AudioStatus === 'function' ? window.__poolsideV7AudioStatus() : null;
  S.audioStatus = status?.status || S.audioStatus;
  receiverActive = true;
  S.receiverStatus = 'Receiver active.';
...
async function playSuno(push = true) {
  ...
  if (S.screen !== 'home' && push) {
    S.musicProvider = 'suno';
    await issueCommand('play', { provider: 'suno', label: 'Play Suno', detail: sourceLabel('suno', S.playlistUrl) }, 'Play command sent to Home receivers.');
    return;
  }
  await ensureReceiverAudio('Suno play');
  S.musicProvider = 'suno';
  if (!hasPlayableSuno()) {
...
async function pauseSuno(push = true) {
  if (S.screen !== 'home' && push) {
    await issueCommand('pause', { label: 'Pause music' }, 'Pause command sent to Home receivers.');
    return;
  }
...
async function stopSuno(push = true) {
  if (S.screen !== 'home' && push) {
    await issueCommand('stop', { label: 'Stop music' }, 'Stop command sent to Home receivers.');
    return;
  }
  await fade(music, 0, 260).catch(() => {});
  music.pause();
...
async function nextSuno(push = true, message = 'Skipped to next Suno track.') {
  if (S.screen !== 'home' && push) {
    await issueCommand('skip', { label: 'Skip track' }, 'Skip command sent to Home receivers.');
    return;
  }
...
async function spotifyLogin() {
  ...
  sessionSet('ppSpotifyReturn', '/?v=6.3-final');
...
async function completeSpotifyLogin() {
  ...
  try {
    ...
    history.replaceState(null, '', sessionGet('ppSpotifyReturn') || '/?v=6.3-final');
    setFeedback(S.screen === 'home' ? 'Spotify connected on this receiver. Tap Activate + Play Spotify.' : 'Spotify connected on this browser. Command mode will still send playback to Home receivers only.', true);
  } catch (error) {
...
async function spotifyAccessToken() {
  ...
  if (!token?.access_token) throw Error('Spotify is not connected on this receiver. Log in on the speaker-connected Home receiver.');
  if (Number(token.expiresAt || 0) > Date.now() + 90000) return token.access_token;
  if (!token.refresh_token) throw Error('Spotify session expired. Log in again on the receiver.');
  const body = new URLSearchParams({
    client_id: S.spotifyClientId.trim(),
...
function registerSpotifyListeners() {
  spotifyPlayer.addListener('ready', ({ device_id }) => {
    ...
    S.spotifyDeviceName = 'Poolside Pulse V7 Receiver';
...
async function primeSpotifyPlayer() {
  ...
  spotifyPrimePromise = (async () => {
    ...
    spotifyPlayer = new Spotify.Player({
      name: 'Poolside Pulse V7 Receiver',
...
async function spotifyTargetDevice() {
  ...
  const named = devices.find(device => /poolside pulse|serenity shores/i.test(device.name || ''));
  const saved = devices.find(device => device.id && device.id === S.spotifyDeviceId);
  const active = devices.find(device => device.is_active);
  const first = named || saved || active || devices[0];
  if (first) {
    S.spotifyDeviceId = first.id;
    S.spotifyDeviceName = first.name || 'Spotify device';
    localSave();
    return first.id;
  }
  return '';
...
async function spotifyActuallyPlaying() {
...
async function playSpotifyUrl(url = S.spotifyUrl, push = true, options = {}) {
  ...
  if (!spotifyLoggedIn()) throw Error('Spotify is not connected on this receiver. Log in on the speaker-connected Home receiver.');
  ...
  const audible = await spotifyActuallyPlaying();
  if (audible) setSpotifyStatus(`Spotify is playing on this receiver: ${sourceLabel('spotify', playUrl)}.`, true);
  else throw Error('Spotify accepted the play command, but this browser has not started audible playback yet. Tap Activate + Play Spotify on the receiver.');
...
async function duckSpotifyForAnnouncement() {
  ...
  const snapshot = { volume: Number(S.spotifyVolume) || 92, wasPlaying: S.intent === 'playing' };
  try {
    if (S.screen !== 'home' || (!spotifyLoggedIn() && !S.spotifyDeviceId)) return null;
    await spotifySetVolume(S.spotifyDuckedVolume);
...
async function restoreSpotifyAfterAnnouncement(snapshot) {
  if (!snapshot) return;
  try {
    await spotifySetVolume(snapshot.volume || S.spotifyVolume);
...
async function playAnnouncementBlob(blob) {
  if (typeof window.__poolsideV7PlayAnnouncementBlob === 'function') {
    return await window.__poolsideV7PlayAnnouncementBlob(blob, { gain: S.announcementGain });
  }
...
async function performAnnouncement(text, options = {}) {
  ...
  if (S.screen !== 'home') throw Error('Announcements play only on Home receivers.');
  speaking = true;
  let voiceBlob = null;
  let aiError = '';
  if (S.voiceMode === 'ai') {
  ...
  try {
    await ensureReceiverAudio('announcement');
    if (voiceBlob) {
    ...
    throw Error(`Receiver could not start voice audio: ${error.message}. Tap Activate Receiver on the speaker-connected device, then send Speak Now again.`);
  } finally {
...
async function triggerWeatherCheck() {
  if (S.screen !== 'home') {
    await issueCommand('weather-check', { label: 'Weather check', detail: 'Manual weather check requested from command.' }, 'Weather check command sent to Home receivers.');
...
function weatherMonitor() {
  if (S.weatherAuto && S.screen === 'home' && receiverActive && !speaking) weather({ announce: true, reason: '5 minute auto scan' });
}
...
function receiverReadiness() {
  ...
  if (S.musicProvider === 'spotify') {
    ...
    return 'Activate Spotify receiver.';
  }
  return hasPlayableSuno() ? 'Suno queue ready.' : 'Import Suno queue.';
}

function header() {
  return `<header class="top"><div class="brand"><div class="brandMark">SS</div><div class="brandText"><b>Serenity Shores</b><small>Poolside Pulse · V7</small></div></div><nav class="modeSwitch"><button id="home" class="${S.screen === 'home' ? 'on' : ''}">Home</button><button id="cmd" class="${S.screen !== 'home' ? 'on' : ''}">Command</button></nav></header>`;
}
...
function homePage() {
  ...
  const error = visibleLastError();
  return `${header()}<main class="home"><section class="hero"><div class="heroText"><p class="eyebrow">Receiver · V7</p><h1>Poolside Pulse</h1><p>Leave this Home screen open on the speaker-connected receiver. This device owns Spotify playback, announcement audio, schedules, and five-minute weather scans.</p><div class="heroActions"><button id="playHome">${esc(mainButton)}</button><button id="skipHome" class="secondary">Skip</button><button id="stopHome" class="secondary">Stop</button><button id="checkWeatherHome" class="secondary">Check Weather</button></div><span class="pill ${receiverActive ? 'good' : 'warn'}">${receiverActive ? 'Receiver active' : 'Tap once to activate sound'}</span></div>${statusCards()}</section><section class="now"><div class="art">SS<span>Pulse</span></div><div><p class="eyebrow">${S.intent === 'playing' ? 'Now Playing' : S.intent === 'paused' ? 'Paused' : 'Station Ready'}</p><h2>${esc(S.musicProvider === 'spotify' ? (S.spotifyNowPlaying || 'Spotify Receiver') : current.title)}</h2><p>${esc(S.musicProvider === 'spotify' ? compactUrl(S.spotifyUrl) : `${current.artist || 'Suno'} · ${current.duration || ''}`)}</p><div class="bar"><i></i></div><p><b>${esc(S.activeMusicLabel || label)}</b></p><p class="muted">${esc(S.feedback || '')}</p>${error ? `<p class="alert warn">${esc(error)}</p>` : ''}</div></section><section class="cards"><div class="card"><h3>Next Scheduled</h3>${next || '<p class="muted">No enabled schedule items.</p>'}</div><div class="card"><h3>Recent Receiver Log</h3>${logRows(5)}</div></section></main>`;
}

function commandPage() {
  const selected = ann();
  return shell(`<section class="hero"><div class="heroText"><p class="eyebrow">Live Control</p><h1>Command</h1><p>Phones and laptops in Command mode only send durable events. Home receivers do the playing, voice, weather checks, volume ducking, and schedule execution.</p></div>${statusCards()}</section><section class="panel"><h2>Receiver Controls</h2><div class="bigControls"><button id="playCmd">Play / Resume</button><button id="pauseCmd" class="secondary">Pause</button><button id="skipCmd" class="secondary">Skip</button><button id="stopCmd" class="danger">Stop</button><button id="checkWeatherCmd" class="secondary">Check Weather</button></div><div class="splitControls"><label>Quick Announcement<textarea id="quickText">${esc(S.quickText || selected.text)}</textarea></label><div><label>Saved Announcement<select id="quickTemplate">${S.anns.map(item => `<option value="${item.id}" ${item.id === S.selected ? 'selected' : ''}>${esc(item.label)}</option>`).join('')}</select></label><div class="buttonStack"><button id="quickPlay">Speak Now</button><button id="quickHold" class="secondary">Speak as Safety Hold</button><button id="lightningNow" class="secondary">Lightning Hold</button><button id="windNow" class="secondary">Wind Umbrellas</button></div></div></div></section><section class="panel"><div class="sectionHead"><h2>Receiver Log</h2><button id="clearLog" class="secondary">Clear Local Log</button></div>${logRows()}</section>`);
}
...
function wire(id, fn) {
  const el = $(id);
  if (!el) return;
  el.onclick = () => Promise.resolve(fn()).catch(error => {
...
function bind() {
  ...
  wire('quickPlay', async () => {
    S.quickText = val('quickText');
    await sendAnnouncement(S.quickText, false, 'Speak Now');
  });
  wire('quickHold', async () => {
    S.quickText = val('quickText');
    await sendAnnouncement(S.quickText, true, 'Safety hold');
  });
  ...
  document.querySelectorAll('[data-play-row]').forEach(button => {
    button.onclick = () => Promise.resolve(playScheduleItem(S.schedule[Number(button.dataset.playRow)])).catch(error => setFeedback(error.message, false));
  });
...
window.addEventListener('poolside-v7-audio-status', event => {
