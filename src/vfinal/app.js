...
function renderReceiver() {
  ...
  if (spotifyRelevant) {
    readiness.push(
      ...
      ['Spotify receiver', spotify.ready && spotify.activationState === 'active', spotify.ready ? 'Connected and activated from a local tap' : spotify.activationState === 'active' ? 'Activated; waiting for Spotify device connection' : 'Tap Connect Spotify Receiver on this device']
    );
  }
  return `
...
function resetScheduleSequence(draft, scheduleId) {
  ...
  const itemBelongsToSchedule = (schedule?.items || []).some(item => String(item.id || '') === String(playback.scheduledItemId || ''));
  if (draft.activeScheduleId === scheduleId && playback.scheduledRunToken && itemBelongsToSchedule) {
    draft.playback = {
      ...playback,
      intent: 'stopped',
...
function renderSettings() {
  ...
  const spotifyStage = !spotify.loggedIn()
    ? '1 of 3 · Login required'
    : !spotify.playerPrepared
      ? spotify.prepareError ? '2 of 3 · SDK setup needs retry' : '2 of 3 · Preparing Spotify SDK'
      : spotify.ready && spotify.activationState === 'active'
        ? '3 of 3 · Spotify receiver ready'
        : '2 of 3 · Tap Connect on this receiver';
  ...
  return `
    ...
    <section class="settingsGrid">
      ...
      <section class="workspacePanel">
        ...
        <div class="policyNote"><strong>Spotify setup:</strong> The Spotify app must list <code>https://serenity-shores-poolside-pulse.vercel.app/</code> as an exact redirect URI, including the final slash. Playback requires Spotify Premium. If the app is in Development Mode, this receiver account must also be an allowed user.</div>
      </section>
