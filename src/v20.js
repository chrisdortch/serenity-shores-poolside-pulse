...
const HANDLED_KEY = 'poolside-pulse-v20-handled-events';
...
const BASE = {
  ...
  v20VolumeDefaultsApplied: V20_VOLUME_DEFAULTS_ID,
...
function normalize(raw) {
  ...
  const importedFromV20 = String(source.version || '') === VERSION || source.v20DefaultsApplied;
  const importedFromRecentPoolside = importedFromV20 || source.v18DefaultsApplied;

  // V20 uses Spotify as the continuous bed. Suno is a cue/announcement source,
  ...
  if (source.v20VolumeDefaultsApplied !== V20_VOLUME_DEFAULTS_ID) {
    ...
    s.setupNotice = '';
    s.spotifyLastError = '';
    s.v20VolumeDefaultsApplied = V20_VOLUME_DEFAULTS_ID;
  }
...
async function runCommand(command) {
  ...
  if (command.type === 'spotify-play') {
    ...
    else throw Error('V20 Suno cues require a pasted Suno or direct audio URL.');
...
function receiverAudioReady() {
  const status = typeof window.__poolsideV20AudioStatus === 'function' ? window.__poolsideV20AudioStatus() : null;
...
async function ensureReceiverAudio(reason = 'receiver action', options = {}) {
  ...
  const attempts = [];
  if (typeof window.__poolsideV20UnlockAudio === 'function') attempts.push(window.__poolsideV20UnlockAudio(reason, unlockOptions));
  if (unlockOptions.audible || typeof window.__poolsideV20UnlockAudio !== 'function') attempts.push(fallbackUnlockReceiverAudio(reason, unlockOptions));
  if (attempts.length) {
...
async function stopReceiverForegroundAudio(reason = 'stop command') {
  ...
  if (typeof window.__poolsideV20StopAudio === 'function') {
    try { stopped = !!window.__poolsideV20StopAudio(reason) || stopped; } catch {}
  }
...
function audioSettingsDetail() {
  ...
  return `Spotify pause/restore for voice; Suno ${audio.sunoVolume}%; voice ${voiceLoudnessLabel(audio.announcementGain)}; speed ${audio.rate}; pitch ${audio.pitch}`;
}
...
async function sendSpotifyVolumeCommand(volume, options = {}) {
  ...
  const event = appendEvent('command', {
    ...
    detail: `Spotify music volume ${safeVolume}%; spoken commands pause music and play through the clear PA voice path.`
  });
  if (!options.live) {
...
async function applySpotifyVolume(push = true, options = {}) {
  ...
  if (S.screen !== 'home' && push) {
    ...
    if (!options.silent) {
      setSpotifyStatus(`Spotify music level ${volume}% sent to receivers. Home pauses music during spoken announcements so voice can take over.`, true);
      renderWhenIdle();
    }
    return;
  }
...
function spotifyIOSVolumeLimitDetail(volume = S.spotifyVolume) {
  return `iPhone browser Spotify volume may not change the physical speaker loudness; ${spotifyVolumePercent(volume)}% is still sent to Spotify, and ${IOS_VOLUME_BRIDGE_FIX_TEXT}`;
}
...
async function setupLoudVoiceReceiver() {
  ...
  storageSet(IOS_VOLUME_BRIDGE_MODE_KEY, IOS_VOLUME_BRIDGE_MODE_ID);
  S.musicProvider = 'spotify';
  S.spotifyVolume = spotifyVolumePercent(S.spotifyVolume || DEFAULT_SPOTIFY_VOLUME);
...
async function playReceiverAudioUrl(url, options = {}) {
  ...
  if (typeof window.__poolsideV20PlayAudioUrl === 'function') {
    return await window.__poolsideV20PlayAudioUrl(raw, {
      label,
      volume,
      gain: volume,
      minGain: 0,
  ...
  if (typeof window.__poolsideV20PlayAnnouncementBlob === 'function') {
    try {
      const response = await fetch(raw, { cache: 'no-store' });
      if (!response.ok) throw Error(`Audio URL returned HTTP ${response.status}`);
      const blob = await response.blob();
      return await window.__poolsideV20PlayAnnouncementBlob(blob, {
        label,
        volume,
        gain: volume,
        minGain: 0,
...
function render() {
  try {
    ...
    app.innerHTML = S.screen === 'home'
      ...
      : !S.admin
        ? login()
        : ({ command: commandPage, music: musicPageV20, party: () => schedulePage('party'), schedule: () => schedulePage('daily'), weather: weatherPage, voice: voicePage, hours: hoursPage }[S.tab] || commandPage)();
    bind();
  } catch (error) {
...
function bind() {
  ...
  wire('useSuno', async () => {
    ...
    S.musicProvider = 'spotify';
    rememberActiveSource('spotify', S.spotifyUrl, 'selected');
    await save('V20 keeps Spotify as the bed music. Paste a Suno URL and use Play Saved Suno Cue for Suno.');
  });
  ...
  wire('makeReceiver', async () => {
    readMusicSettings();
    setFeedback('Command stays Command in V20. Open Home on the speaker-connected phone, then tap Start Receiver + Play Spotify there.', true);
  });
...
window.addEventListener('poolside-v20-audio-status', event => {
