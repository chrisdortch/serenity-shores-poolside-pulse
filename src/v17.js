const VERSION = '17';
const PIN = '7900';
const KEY = 'poolside-pulse-v17';
const DEVICE_KEY = 'poolside-pulse-v17-device-id';
const HANDLED_KEY = 'poolside-pulse-v17-handled-events';
const LOG_CLEAR_KEY = 'poolside-pulse-v17-log-cleared-at';
const RECEIVER_SESSION_KEY = 'poolside-pulse-v17-receiver-session-started-at';
const SPOTIFY_TOKEN_KEY = 'poolside-pulse-v17-spotify-token';
const APP_QUERY = '?v=17';
...
function normalize(raw) {
  ...
  const importedFromV17 = String(source.version || '') === VERSION || source.v17DefaultsApplied;
...
function receiverAudioReady() {
  const status = typeof window.__poolsideV17AudioStatus === 'function' ? window.__poolsideV17AudioStatus() : null;
...
async function ensureReceiverAudio(reason = 'receiver action', options = {}) {
  ...
  const attempts = [];
  if (typeof window.__poolsideV17UnlockAudio === 'function') attempts.push(window.__poolsideV17UnlockAudio(reason, unlockOptions));
  if (unlockOptions.audible || typeof window.__poolsideV17UnlockAudio !== 'function') attempts.push(fallbackUnlockReceiverAudio(reason, unlockOptions));
  if (attempts.length) {
  ...
  S.receiverStatus = unlocked ? 'Receiver audio ready.' : 'Receiver online; tap Start Speaker Phone once for sound.';
  ...
  if (!unlocked) {
    ...
    S.setupNotice = detail && !/not been activated yet/i.test(detail)
      ? detail
      : 'Tap Start Speaker Phone on this speaker-connected phone once. iPhone browsers require that tap before music or announcements can be heard.';
  }
  localSave();
...
async function testReceiverTone(reason = 'iPhone receiver test tone') {
  ...
  try {
    ...
    if (!ok) throw actionNeededError(S.setupNotice || 'Tap Start Speaker Phone on this iPhone before sending sound.');
    setFeedback('Receiver audio test completed on this phone.', true);
    logEvent('receiver', 'Receiver audio test', 'Start Speaker Phone test tone completed on this Home device.');
...
async function playSuno(push = true) {
  ...
  music.volume = musicGain();
  await music.play();
  await fade(music, musicGain(), 450);
  S.intent = 'playing';
...
async function completeSpotifyLogin() {
  ...
  try {
    ...
    setFeedback(S.screen === 'home' ? 'Spotify connected on this receiver. Tap Start Speaker Phone.' : 'Spotify connected on this browser. Command mode will send playback to all active receivers.', true);
  } catch (error) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    const timer = setInterval(() => {
      if (spotifyPlayerReady && spotifyWebDeviceId) {
        ...
        clearInterval(timer);
        reject(Error('Spotify receiver did not report ready. Tap Start Speaker Phone on the speaker-connected receiver.'));
      }
    }, 140);
  });
}
...
function registerSpotifyListeners() {
  spotifyPlayer.addListener('ready', ({ device_id }) => {
    ...
    S.spotifyDeviceName = 'Poolside Pulse V17 Receiver';
  ...
  spotifyPlayer.addListener('not_ready', () => {
    ...
    setSpotifyStatus('Spotify receiver went offline. Leave Home open and tap Start Speaker Phone again.', false);
    localSave();
    renderWhenIdle();
  });
  spotifyPlayer.addListener('autoplay_failed', () => {
    setSpotifyStatus('Spotify is ready, but this browser blocked autoplay. Tap Start Speaker Phone on the receiver.', false);
  });
...
async function primeSpotifyPlayer() {
  ...
  spotifyPrimePromise = (async () => {
    ...
    spotifyPlayer = new Spotify.Player({
      name: 'Poolside Pulse V17 Receiver',
...
async function startSpotifyReceiver({ fromTap = false } = {}) {
  ...
  try {
    ...
    if (current?.deviceId) {
      ...
      setSpotifyStatus(`Spotify did not start on this iPhone browser: ${error.message || error}. Keep Home open and tap Start Speaker Phone again.`, false);
      localSave();
      throw error;
    }
  }
  S.spotifyDeviceId = deviceId;
  S.spotifyDeviceName = S.spotifyDeviceName || 'Poolside Pulse V17 Receiver';
  S.spotifyReceiverReadyAt = Date.now();
  S.receiverStatus = 'Spotify receiver active.';
  S.receiverLastSeen = stamp();
  receiverActive = true;
  ...
  if (S.screen === 'home') {
    ...
    if (spotifyLoggedIn()) {
      try {
        ...
        throw actionNeededError(`Spotify receiver is not ready on this phone yet. Tap Start Speaker Phone on the speaker-connected Home phone. ${error.message || ''}`.trim());
      }
    }
...
async function playSpotifyUrl(url = S.spotifyUrl, push = true, options = {}) {
  ...
  if (audible) {
    ...
    const message = 'Spotify accepted the command, but this Home receiver is not the audible Spotify device yet. Tap Start Speaker Phone on the speaker-connected phone once.';
    S.intent = 'stopped';
    S.spotifyNeedsTap = true;
...
async function spotifyPause(push = true) {
  ...
  if (!deviceId) throw actionNeededError('Spotify is playing, but this receiver could not identify the active Spotify device. Tap Start Speaker Phone once, then press Stop again.');
...
async function performSunoAnnouncement(trackIndex, options = {}) {
  ...
  try {
    ...
    throw actionNeededError(`Receiver could not start the Suno announcement: ${error.message}. Tap Start Speaker Phone on the speaker-connected phone, then send it again.`);
  } finally {
    speaking = false;
    localSave();
    renderWhenIdle();
...
async function playAnnouncementBlob(blob) {
  if (typeof window.__poolsideV17PlayAnnouncementBlob === 'function') {
    return await window.__poolsideV17PlayAnnouncementBlob(blob, { gain: S.announcementGain });
  }
...
function playDeviceSpeech(message) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      if (!started && !settled) {
        settled = true;
        reject(Error('Device speech did not start. Tap Start Speaker Phone once, then send Speak Now again.'));
      }
    }, 4200);
...
async function performAnnouncement(text, options = {}) {
  ...
  try {
    ...
    throw actionNeededError(`Receiver could not start voice audio: ${error.message}. Tap Start Speaker Phone on the speaker-connected phone, then send Speak Now again.`);
  } finally {
    await restoreSpotifyAfterAnnouncement(spotifySnapshot);
    await restoreSunoAfterAnnouncement(sunoSnapshot);
    speaking = false;
...
function receiverReadiness() {
  ...
  if (!receiverSessionStartedAt()) return 'Tap Start Speaker Phone to begin a fresh receiver session.';
  if (!receiverAudioReady()) return 'Tap Start Speaker Phone on this speaker-connected phone.';
...
function receiverNotice() {
  let message = '';
  if (!receiverSessionStartedAt() || !receiverAudioReady()) message = 'Tap the big Start Speaker Phone button. That one tap starts a fresh receiver session, ignores old commands, and unlocks iPhone audio.';
  ...
  else if (S.musicProvider === 'spotify' && !spotifyDeviceReady()) message = 'Tap Start Speaker Phone or the Spotify device ready TODO row to make this phone the speaker device.';
...
window.addEventListener('poolside-v17-audio-status', event => {
  if (event.detail?.status) {
    ...
    if (helperAudioReady(event.detail)) {
      ...
      receiverActive = false;
      S.receiverStatus = 'Receiver online; tap Start Speaker Phone once for sound.';
