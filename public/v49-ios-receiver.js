(() => {
  ...
  function note(text) {
    try {
      window.__pulseV49Status = text;
      const box = document.getElementById('feedbackBox');
      if (box && !box.innerHTML.includes(text)) box.innerHTML += '<br><b>V4.9 iPhone:</b> ' + text;
    } catch {}
  }
  ...
  function fade(media, target, ms = 700) {
    try {
      ...
      const steps = 14;
      let i = 0;
      clearInterval(media.__pulseV49FadeTimer);
      media.__pulseV49FadeTimer = setInterval(() => {
        i += 1;
        try { media.volume = Math.max(0, Math.min(1, start + (target - start) * (i / steps))); } catch {}
        if (i >= steps) clearInterval(media.__pulseV49FadeTimer);
  ...
  if (!HTMLMediaElement.prototype.__pulseV49Play) {
    ...
    HTMLMediaElement.prototype.__pulseV49Play = true;
  }

  if ('speechSynthesis' in window && !window.speechSynthesis.__pulseV49Speak) {
    ...
    window.speechSynthesis.__pulseV49Speak = true;
  }

  function rewriteVersion() {
    try {
      document.title = 'Serenity Shores Poolside Pulse V4.9';
      const root = document.getElementById('app');
      if (root) root.dataset.version = '4.9';
      const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
      const nodes = [];
      while (walker.nextNode()) nodes.push(walker.currentNode);
      nodes.forEach(n => {
        if (n.nodeValue) n.nodeValue = n.nodeValue
          .replaceAll('V4.8','V4.9').replaceAll('Version 4.8','Version 4.9')
          .replaceAll('V4.7','V4.9').replaceAll('Version 4.7','Version 4.9')
          .replaceAll('V4.6','V4.9').replaceAll('Version 4.6','Version 4.9')
          .replaceAll('V4.5','V4.9').replaceAll('Version 4.5','Version 4.9')
          .replaceAll('V4.4','V4.9').replaceAll('Version 4.4','Version 4.9')
          .replaceAll('V4.2','V4.9').replaceAll('Version 4.2','Version 4.9')
          .replaceAll('V4.0','V4.9').replaceAll('Version 4.0','Version 4.9');
      });
    } catch {}
  }

  window.__poolsidePulseUnlockAudio = unlockReceiverAudio;
  window.__poolsidePulseV49Audio = { version: PATCH_VERSION, useMuteDuck: USE_MUTE_DUCK, isiPhoneLike: isiPhoneLike(), isSafariLike: isSafariLike() };
