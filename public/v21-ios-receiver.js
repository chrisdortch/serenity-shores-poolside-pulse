(() => {
  ...
  function dispatchStatus() {
    try {
      window.dispatchEvent(new CustomEvent('poolside-v21-audio-status', { detail: status() }));
    } catch {}
  }
  ...
  function playbackOptions(options = {}) {
    ...
    return {
      ...
      minGain,
      maxGain,
      voiceGain: clampNumber(raw.voiceGain, 0, 500, 500),
  ...
  function voiceLoudnessSettings(buffer, options) {
    ...
    const managerStrength = Math.max(0, Math.min(1, clampNumber(options.voiceGain, 0, 500, 500) / 500));
    const strength = Math.max(requestedStrength, managerStrength);
    const targetRms = 0.18 + strength * 0.16;
    const rmsGain = analysis.rms ? targetRms / analysis.rms : 1;
    const peakGain = analysis.peak ? 2.7 / analysis.peak : rmsGain;
    const driveGain = Math.max(1, Math.min(12, rmsGain, peakGain));
    return {
      driveGain,
      outputGain: 0.94 + strength * 0.04,
      threshold: -21 - strength * 7,
      ratio: 10 + strength * 10,
      knee: 7,
      peak: analysis.peak,
      rms: analysis.rms
    };
  ...
  window.__poolsideV21UnlockAudio = unlock;
  window.__poolsideV21AudioStatus = status;
  window.__poolsideV21PlayAnnouncementBlob = playBlob;
  window.__poolsideV21PlayAudioUrl = playAudioUrl;
  window.__poolsideV21PlayTestTone = playTestTone;
  window.__poolsideV21StopAudio = stopActiveAudio;
