(() => {
  ...
  function dispatchStatus() {
    try {
      const detail = status();
      window.dispatchEvent(new CustomEvent('poolside-v13-audio-status', { detail }));
  ...
  async function unlock(reason = 'receiver tap', options = {}) {
    ...
    unlockPromise = (async () => {
      ...
      lastStatus = options.quiet
        ? 'Tap Activate Receiver to unlock audio on this iPhone.'
  ...
  async function playWithWebAudio(blob, gainValue) {
    ...
    if (ctx.state !== 'running') throw Error(`Web Audio is ${ctx.state}; tap Activate Receiver on this phone.`);
  ...
  window.__poolsideV13UnlockAudio = unlock;
  window.__poolsideV13AudioStatus = status;
  window.__poolsideV13PlayAnnouncementBlob = playBlob;
  window.__poolsideV13PlayTestTone = playTestTone;
