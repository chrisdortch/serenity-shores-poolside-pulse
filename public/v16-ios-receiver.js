(() => {
  ...
  function dispatchStatus() {
    try {
      const detail = status();
      window.dispatchEvent(new CustomEvent('poolside-v16-audio-status', { detail }));
      window.dispatchEvent(new CustomEvent('poolside-v9-audio-status', { detail }));
    } catch {}
  }
  ...
  async function unlock(reason = 'receiver tap', options = {}) {
    ...
    unlockPromise = (async () => {
      ...
      lastStatus = options.quiet
        ? 'Tap Start Sound Station to unlock audio on this iPhone.'
  ...
  async function playWithWebAudio(blob, gainValue) {
    ...
    if (ctx.state !== 'running') throw Error(`Web Audio is ${ctx.state}; tap Start Sound Station on this phone.`);
  ...
  window.__poolsideV16UnlockAudio = unlock;
  window.__poolsideV16AudioStatus = status;
  window.__poolsideV16PlayAnnouncementBlob = playBlob;
  window.__poolsideV16PlayTestTone = playTestTone;
